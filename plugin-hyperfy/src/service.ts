...
- Manage multi-agent interactions in virtual environments
  ...
  async connect(config: { wsUrl: string; authToken?: string; worldId: UUID }): Promise<void> {
    ...
    try {
      ...
      if (this.dynamicActionLoader && this.world) {
        const discoveredActions = await this.dynamicActionLoader.discoverActions(this.world);
        console.info(`[HyperfyService] Discovered ${discoveredActions.length} dynamic actions`);

        for (const actionDescriptor of discoveredActions) {
          await this.dynamicActionLoader.registerAction(actionDescriptor, this.runtime);
        }
      }
  ...
  async handleDisconnect(): Promise<void> {
    ...
    if (this.dynamicActionLoader) {
      ...
      for (const [actionName, _] of registeredActions) {
        await this.dynamicActionLoader.unregisterAction(actionName, this.runtime);
      }
    ...
    for (const [contentId, content] of this.loadedContent) {
      if (content && typeof content.uninstall === 'function') {
        try {
          ...
          console.error(`[HyperfyService] Error uninstalling content ${contentId}:`, error);
        }
      }
    }
   ...
   */
  async loadUGCContent(contentId: string, contentBundle: any): Promise<boolean> {
    if (!this.world) {
    ...
    if (this.loadedContent.has(contentId)) {
      console.warn(`[HyperfyService] Content ${contentId} already loaded. Unloading first...`);
    ...
    try {
      ...
      if (typeof contentBundle.install === 'function') {
        ...
        if (contentBundle.actions && Array.isArray(contentBundle.actions)) {
          console.info(`[HyperfyService] Registering ${contentBundle.actions.length} actions from ${contentId}`);
          for (const action of contentBundle.actions) {
            ...
            } else if (this.runtime.actions && Array.isArray(this.runtime.actions)) {
        ...
        if (contentBundle.providers && Array.isArray(contentBundle.providers)) {
          console.info(`[HyperfyService] Registering ${contentBundle.providers.length} providers from ${contentId}`);
          for (const provider of contentBundle.providers) {
            ...
            } else if (this.runtime.providers && Array.isArray(this.runtime.providers)) {
        ...
        if (contentBundle.dynamicActions && this.dynamicActionLoader) {
          console.info(`[HyperfyService] Discovering dynamic actions from ${contentId}`);
        ...
        console.info(`[HyperfyService] UGC content ${contentId} loaded successfully`);
        return true;
      } else {
      ...
      console.error(`[HyperfyService] Failed to load UGC content ${contentId}:`, error);
      return false;
    }
  }
  ...
  async unloadUGCContent(contentId: string): Promise<boolean> {
    ...
    try {
      ...
      if (content.actions && Array.isArray(content.actions)) {
        console.info(`[HyperfyService] Unregistering ${content.actions.length} actions from ${contentId}`);
      ...
      if (content.providers && Array.isArray(content.providers)) {
        console.info(`[HyperfyService] Unregistering ${content.providers.length} providers from ${contentId}`);
      ...
      if (content.dynamicActions && this.dynamicActionLoader) {
        console.info(`[HyperfyService] Unregistering ${content.dynamicActions.length} dynamic actions from ${contentId}`);
      ...
      console.info(`[HyperfyService] UGC content ${contentId} unloaded successfully`);
      return true;
    } catch (error) {
      console.error(`[HyperfyService] Failed to unload UGC content ${contentId}:`, error);
      return false;
    }
  }
