...
export class VisualVerifier {
    ...
    async initialize(): Promise<void> {
        ...
        if (this.config.useColorDetection || this.config.testMode) {
            ...
            try {
                const hyperfyPath = require.resolve('@elizaos/hyperfy');
                const hyperfyDir = hyperfyPath.substring(0, hyperfyPath.lastIndexOf('/'));
                const ColorDetectorModule = require(`${hyperfyDir}/rpg/testing/ColorDetector`);
                ...
                logger.warn('[VisualVerifier] Could not load ColorDetector from Hyperfy:', error);
     ...
     */
    registerCheckpoints(scenarioName: string, checkpoints: VisualCheckpoint[]): void {
        this.checkpoints.set(scenarioName, checkpoints);
        logger.debug(`[VisualVerifier] Registered ${checkpoints.length} checkpoints for scenario: ${scenarioName}`);
    }
    ...
    async verifyScenario(scenarioName: string): Promise<VisualTestResult[]> {
        ...
        if (!checkpoints) {
            logger.warn(`[VisualVerifier] No checkpoints registered for scenario: ${scenarioName}`);
            return [];
        }
     ...
     */
    async verifyCheckpoint(checkpoint: VisualCheckpoint): Promise<VisualTestResult> {
        try {
            ...
            if (this.colorDetector && screenshotPath) {
                const detectedEntities = await this.colorDetector.detectEntitiesInImage(screenshotPath);
                ...
                if (targetEntity) {
                    ...
                    if (checkpoint.expectedPosition && targetEntity.positions.length > 0) {
                        ...
                        positionMatch = distance <= (checkpoint.expectedPosition.range || 50);
                    }
            ...
            logger.error(`[VisualVerifier] Error verifying checkpoint ${checkpoint.name}:`, error);
            return {
                checkpoint: checkpoint.name,
                success: false,
                details: { found: false },
    ...
    private checkWorldState(checkpoint: VisualCheckpoint): boolean {
        ...
        for (const [id, entity] of this.world.entities.items) {
            if (entity.type === checkpoint.entityType ||
    ...
    private createMockColorDetector(): any {
        return {
            ...
            detectEntitiesInImage: async (imagePath: string) => {
                ...
                logger.debug(`[MockColorDetector] Would detect entities in: ${imagePath}`);
                return [];
            },
            detectColorAtPosition: async (position: any, expectedColor: string) => {
                logger.debug(`[MockColorDetector] Would check color at position: ${JSON.stringify(position)}`);
