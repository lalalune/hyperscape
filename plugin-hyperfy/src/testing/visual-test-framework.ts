...
import { RPGStateManager } from '../managers/rpg-state-manager';
import type { RPGPlayerState } from '../managers/rpg-state-manager';
...
export class VisualTestFramework {
   ...
   */
  async verifyVisual(checks: VisualCheck[]): Promise<{ passed: boolean; failures: string[] }> {
    ...
    for (const check of checks) {
      try {
        ...
        if (!template) {
          failures.push(`No visual template for entity type: ${check.entityType}`);
          continue;
        }
        ...
        const detectedEntities = await this.colorDetector.detectEntities(expectedColor, {
        ...
        if (check.shouldExist && !entityExists) {
          failures.push(`Expected ${check.entityType} (color: ${expectedColor}) not found`);
        } else if (!check.shouldExist && entityExists) {
          failures.push(`Unexpected ${check.entityType} (color: ${expectedColor}) found`);
        }
        ...
        if (check.position && entityExists) {
          ...
          if (distance > 5) { // 5 unit tolerance
            failures.push(`${check.entityType} at wrong position. Expected: ${JSON.stringify(check.position)}, Found: ${JSON.stringify(entity.position)}`);
          }
        }
      } catch (error) {
        failures.push(`Visual check failed for ${check.entityType}: ${error.message}`);
      }
    }
   ...
   */
  async verifyState(checks: StateCheck[]): Promise<{ passed: boolean; failures: string[] }> {
    const failures: string[] = [];
    ...
    for (const check of checks) {
      try {
        ...
        const passed = this.compareValues(actualValue, check.expectedValue, check.operator);

        if (!passed) {
          failures.push(`State check failed: ${check.property} expected ${check.operator} ${check.expectedValue}, got ${actualValue}`);
        }
      } catch (error) {
        failures.push(`State check error for ${check.property}: ${error.message}`);
      }
    }
  ...
  async captureScreenshot(name: string): Promise<string> {
    ...
    if (!puppeteerManager) {
      logger.warn('[VisualTestFramework] PuppeteerManager not available for screenshot');
      return '';
    }
   ...
   */
  async runTest(name: string, verification: TestVerification): Promise<TestResult> {
...
export class RPGTestHelpers {
   ...
   */
  static combatVerification(targetEntity: string, minDamage: number): TestVerification {
    return {
      type: 'both',
      visualChecks: [
        { entityType: targetEntity, expectedColor: null, shouldExist: true },
        { entityType: 'special.damage_indicator', expectedColor: null, shouldExist: true }
      ],
      stateChecks: [
        { property: 'combat.inCombat', expectedValue: true, operator: 'equals' },
        { property: 'combat.target', expectedValue: targetEntity, operator: 'contains' }
      ],
      screenshot: true
    };
  }
   ...
   */
  static inventoryVerification(itemName: string, expectedQuantity: number): TestVerification {
    return {
      type: 'state',
      stateChecks: [
        { property: 'inventory.items', expectedValue: itemName, operator: 'contains' }
      ]
    };
  }
   ...
   */
  static skillVerification(skillName: string, minLevel: number): TestVerification {
    return {
      type: 'state',
      stateChecks: [
        { property: `skills.${skillName}.level`, expectedValue: minLevel, operator: 'greater' }
      ]
    };
  }
  ...
  static questVerification(questName: string): TestVerification {
    return {
      type: 'state',
      stateChecks: [
        { property: 'quests.completed', expectedValue: questName, operator: 'contains' }
      ]
    };
  }
   ...
   */
  static positionVerification(expectedPos: { x: number; y: number; z: number }, tolerance: number = 5): TestVerification {
    return {
      type: 'both',
      visualChecks: [
        { entityType: 'special.player', expectedColor: null, position: expectedPos, shouldExist: true }
      ],
      stateChecks: [
        { property: 'location.coordinates.x', expectedValue: expectedPos.x - tolerance, operator: 'greater' },
        { property: 'location.coordinates.x', expectedValue: expectedPos.x + tolerance, operator: 'less' }
      ]
    };
  }
} 