...
export class EnhancedObserver {
  ...
  async highlightAction(actionName: string, duration: number = 3000): Promise<void> {
    ...
    if (this.puppeteerManager) {
      await this.puppeteerManager.page?.evaluate((action, dur) => {
  ...
  async showAgentThoughts(thought: string): Promise<void> {
    ...
    if (this.puppeteerManager) {
      await this.puppeteerManager.page?.evaluate((thoughtText) => {
  ...
  private async openObservationWindow(): Promise<void> {
    ...
    try {
      ...
      await this.puppeteerManager.page?.setViewport({
        width: 1920,
        height: 1080
      });
  ...
  private async injectObservationUI(): Promise<void> {
    ...
    await this.puppeteerManager.page?.evaluate(() => {
  ...
  /**
   * Updates observation stats
   */
  private async updateObservationStats(): Promise<void> {
    if (!this.puppeteerManager || !this.config.overlayStats) return;
    ...
    await this.puppeteerManager.page?.evaluate((stats) => {
      const overlay = document.getElementById('observer-stats');
      if (overlay) {
        overlay.innerHTML = `
          <h4 style="margin: 0 0 10px 0;">Agent Stats</h4>
          <div>Level: ${stats.level || 'N/A'}</div>
          <div>Health: ${stats.health || 'N/A'}</div>
          <div>Position: ${stats.position || 'N/A'}</div>
          <div>Zone: ${stats.zone || 'N/A'}</div>
          <div>Actions: ${stats.totalActions || 0}</div>
          <div>Distance: ${stats.distance || 0}m</div>
        `;
      }
    }, {
      level: playerState?.level,
      health: playerState ? `${playerState.health.current}/${playerState.health.max}` : null,
      position: world?.entities?.player?.data?.position ? 
        `(${Math.round(world.entities.player.data.position.x)}, ${Math.round(world.entities.player.data.position.y)}, ${Math.round(world.entities.player.data.position.z)})` : null,
      zone: playerState?.location.zone,
      totalActions: this.recording?.metrics.totalActions,
      distance: this.recording?.metrics.distanceTraveled?.toFixed(1)
    });
  }
  ...
  private async logAction(action: string, success: boolean): Promise<void> {
    ...
    await this.puppeteerManager.page?.evaluate((actionText, isSuccess) => {
  ...
  private async observationTick(): Promise<void> {
    try {
      ...
      await this.updateObservationStats();
      ...
      this.lastPosition = currentPos ? { ...currentPos } : null;
      ...
      if (this.recording && this.puppeteerManager) {
        ...
        if (shouldScreenshot) {
          const screenshot = await this.puppeteerManager.page?.screenshot({ encoding: 'base64' });
          if (screenshot) {
