...
const RuntimeTest: React.FC = () => {
  ...
  const runAllTests = async () => {
    ...
    results.push(await runTest('Player Creation and Movement', async () => {
      // Create players
      const players = [
        { id: 'p1', name: 'Red', color: 'red', colorHex: 0xff0000, role: 'impostor' as const },
        { id: 'p2', name: 'Blue', color: 'blue', colorHex: 0x0000ff, role: 'crewmate' as const },
        { id: 'p3', name: 'Green', color: 'green', colorHex: 0x00ff00, role: 'crewmate' as const },
        { id: 'p4', name: 'Yellow', color: 'yellow', colorHex: 0xffff00, role: 'crewmate' as const },
        { id: 'p5', name: 'Purple', color: 'purple', colorHex: 0xff00ff, role: 'impostor' as const }
      ];
      ...
      if (gameState.players.size !== 5) throw new Error('Failed to create 5 players');
    }));

    // Test 2: Task System
    results.push(await runTest('Task Creation and Progress', async () => {
      // Create tasks
      const tasks = [
        { id: 't1', name: 'Fix Wiring', position: { x: -15, y: 0, z: -15 } },
        { id: 't2', name: 'Download Data', position: { x: 15, y: 0, z: -15 } },
        { id: 't3', name: 'Calibrate Distributor', position: { x: 0, y: 0, z: 0 } }
      ];
      ...
      const crewmates = Array.from(gameState.players.values())
      ...
      if (gameState.tasks.size !== 3) throw new Error('Failed to create tasks');
    }));

    // Test 3: Kill Mechanics
    results.push(await runTest('Kill Mechanics', async () => {
      const impostor = Array.from(gameState.players.values())
        .find(p => p.role === 'impostor' && p.alive);
      const victim = Array.from(gameState.players.values())
        .find(p => p.role === 'crewmate' && p.alive);
      ...
      if (!impostorMesh || !victimMesh) {
        ...
        const geometry = new THREE.CapsuleGeometry(0.5, 1, 4, 8);
        const impostorMat = new THREE.MeshStandardMaterial({ color: 0xff0000 });
    ...
    results.push(await runTest('Meeting System', async () => {
      // Report body
      const reporter = Array.from(gameState.players.values())
        .find(p => p.alive);
    ...
    results.push(await runTest('Win Condition Detection', async () => {
      ...
      const totalTasks = gameState.tasks.size * 
        Array.from(gameState.players.values()).filter(p => p.role === 'crewmate').length;
      const completedTasks = Array.from(gameState.tasks.values())
        .reduce((sum, task) => sum + task.completedBy.size, 0);
      ...
      const crewmates = Array.from(gameState.players.values())
        .filter(p => p.role === 'crewmate');
      const impostors = Array.from(gameState.players.values())
          ...
          <div>
            ...
            {testResults.map((result, index) => (
              ...
              >
                ...
                {result.error && (
                  <div style={{ fontSize: '12px', color: '#ff7675', marginTop: '5px' }}>
                    Error: {result.error}
                  </div>
                )}
              </div>
            ...
            <div style={{ marginTop: '20px', padding: '10px', background: '#2d3436', borderRadius: '5px' }}>
              <h4>Summary</h4>
              ...
              <p>Success Rate: {Math.round(testResults.filter(r => r.passed).length / testResults.length * 100)}%</p>
            </div>
          </div>
        )}
      </div>
