...
describe('UGC Content Loading', () => {
  ...
  describe('loadUGCContent', () => {
    it('should successfully load content with actions and providers', async () => {
      ...
      expect(mockContentBundle.install).toHaveBeenCalledWith(mockWorld, mockRuntime);
    ...
    it('should handle content with dynamic actions', async () => {
      ...
      const mockDynamicLoader = new DynamicActionLoader(mockRuntime as IAgentRuntime);
      ...
      const result = await service.loadUGCContent('dynamic-content', mockContentBundle);

      expect(result).toBe(true);
      expect(registerActionSpy).toHaveBeenCalledWith(dynamicActions[0], mockRuntime);
    });
