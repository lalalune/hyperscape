...
export interface RPGPlayerData {
  id: string
  hyperfyPlayerId: string
  name: string
  skills: {
...
export interface RPGEquipmentSlot {
  slotName: string
  itemId: number | null
  quantity: number
  itemData: any
...
// World chunk persistence types
export interface RPGWorldChunk {
  chunkX: number
  chunkZ: number
  biome: string
  heightData: number[] | null
  resourceStates: Record<string, any>
  mobSpawnStates: Record<string, any>
  playerModifications: Record<string, any>
  chunkSeed?: number
...
// Player session management types
export interface RPGPlayerSession {
  sessionId: string
  playerId: string
  playerToken: string
  startTime: Date
...
// Remove duplicate RPGItem interface - use the one defined earlier with proper structure
