    ...
    userCharacters.map(async (char) => {
      const [invCount, equipCount, bankCount] = await Promise.all([
        db
          .select({ count: count() })
          .from(inventory)
          .where(eq(inventory.playerId, char.id)),
        db
          .select({ count: count() })
          .from(equipment)
          .where(eq(equipment.playerId, char.id)),
        db
          .select({ count: count() })
          .from(bankStorage)
          .where(eq(bankStorage.playerId, char.id)),
      ]);
  ...
  if (characterIds.length > 0) {
    const [invTotal, equipTotal, bankTotal, sessionsData] = await Promise.all([
      db
        .select({ count: count() })
        .from(inventory)
        .where(inArray(inventory.playerId, characterIds)),
      db
        .select({ count: count() })
        .from(equipment)
        .where(inArray(equipment.playerId, characterIds)),
      db
        .select({ count: count() })
        .from(bankStorage)
        .where(inArray(bankStorage.playerId, characterIds)),
      db
        .select({
          count: count(),
        ...
        .from(playerSessions)
        .where(inArray(playerSessions.playerId, characterIds)),
    ]);
...
export async function deleteUser(
  ...
  if (characterIds.length > 0) {
    ...
    deletedCounts.inventory =
      (invResult as unknown as { rowCount?: number }).rowCount ||
      characterIds.length;

    // Delete equipment
    const equipResult = await db
      .delete(equipment)
      .where(inArray(equipment.playerId, characterIds));
    deletedCounts.equipment =
      (equipResult as unknown as { rowCount?: number }).rowCount || 0;

    // Delete bank storage
    const bankResult = await db
      .delete(bankStorage)
      .where(inArray(bankStorage.playerId, characterIds));
    deletedCounts.bankStorage =
      (bankResult as unknown as { rowCount?: number }).rowCount || 0;

    // Delete player sessions
    const sessResult = await db
      .delete(playerSessions)
      .where(inArray(playerSessions.playerId, characterIds));
    deletedCounts.playerSessions =
      (sessResult as unknown as { rowCount?: number }).rowCount || 0;

    // Delete NPC kills
    const killsResult = await db
      .delete(npcKills)
      .where(inArray(npcKills.playerId, characterIds));
    deletedCounts.npcKills =
      (killsResult as unknown as { rowCount?: number }).rowCount || 0;
  }
  ...
  console.log(
    "[Admin] Deleted user with cascading data:",
    userId,
    deletedCounts,
  );
