...
export function CharacterDetailModal({
  ...
  const combatSkills = [
    {
      name: "Attack",
      level: character.attackLevel,
      xp: character.attackXp,
      icon: Swords,
    },
    {
      name: "Strength",
      level: character.strengthLevel,
      xp: character.strengthXp,
      icon: TrendingUp,
    },
    {
      name: "Defense",
      level: character.defenseLevel,
      xp: character.defenseXp,
      icon: Shield,
    },
    {
      name: "Constitution",
      level: character.constitutionLevel,
      xp: character.constitutionXp,
      icon: Heart,
    },
    {
      name: "Ranged",
      level: character.rangedLevel,
      xp: character.rangedXp,
      icon: Target,
    },
  ];
  ...
  const gatheringSkills = [
    ...
    },
    {
      ...
      xp: character.woodcuttingXp,
      icon: Axe,
    },
    {
      name: "Fishing",
      level: character.fishingLevel,
      xp: character.fishingXp,
      icon: Fish,
    },
    {
      name: "Firemaking",
      level: character.firemakingLevel,
      xp: character.firemakingXp,
      icon: Flame,
    },
    {
      name: "Cooking",
      level: character.cookingLevel,
      xp: character.cookingXp,
      icon: ChefHat,
    },
  ];
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-start gap-4">
          <div className="w-16 h-16 rounded-full bg-(--bg-tertiary) flex items-center justify-center">
            {character.isAgent ? (
              <Bot className="h-8 w-8 text-(--accent-secondary)" />
            ) : (
        ...
        {character.isAgent && character.agentMapping && (
          <Card className="bg-linear-to-r from-(--accent-secondary)/10 to-(--accent-tertiary)/10 border-(--accent-secondary)">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <CardContent className="space-y-3">
              ...
              {character.wallet && (
                <div>
                  ...
                  <p className="text-xs font-mono text-(--text-primary)">
                    {character.wallet.slice(0, 6)}...
                    {character.wallet.slice(-4)}
                  </p>
                </div>
              )}
              <div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-1 text-xl font-bold text-(--accent-secondary)">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div>
                <p className="text-sm text-(--text-secondary)">Playtime</p>
              ...
              <div>
                <p className="text-sm text-(--text-secondary)">NPC Kills</p>
                <div className="flex items-center gap-1 text-xl font-bold text-(--text-primary)">
                  <Trophy className="h-5 w-5" />
                  {character.kills?.reduce(
                    (acc, k) => acc + (k.killCount || 0),
                    0,
                  ) || 0}
                </div>
              </div>
        ...
        <Card className="bracket-corners">
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Axe className="h-5 w-5 text-(--color-success)" />
              Gathering Skills
            </CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {character.equipment
                    ...
                    .map((item, idx) => (
                      <div
                        key={idx}
                        className="p-3 bg-(--bg-secondary) border border-(--border-primary) rounded"
                      >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {character.kills
                  ...
                  .map((kill, idx) => (
                    <div
                      key={idx}
                      className="p-3 bg-(--bg-secondary) border border-(--border-primary) rounded text-center"
                    >
                      <p className="text-sm font-medium text-(--text-primary)">
                        {kill.npcId}
                      </p>
            ...
            <CardContent>
              <div className="grid grid-cols-3 md:grid-cols-6 gap-2 max-h-64 overflow-y-auto">
                {character.inventory.slice(0, 24).map((item, idx) => (
                  <div
                    key={idx}
                    className="p-2 bg-(--bg-secondary) border border-(--border-primary) rounded text-center"
                  >
        ...
        {character.bank && character.bank.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                Bank ({character.bank.length} items)
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-3 md:grid-cols-6 gap-2 max-h-64 overflow-y-auto">
