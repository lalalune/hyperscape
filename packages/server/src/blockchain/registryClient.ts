...
export class RegistryClient {
  ...
  async registerPlayer(playerAddress: string): Promise<number> {
    ...
    const event = receipt.logs.find(
      (log: ethers.Log) =>
        log.topics[0] ===
        ethers.id("Registered(uint256,address,uint256,uint256,string)"),
    );
  ...
  async registerCharacter(
    ...
    console.log(
      `[RegistryClient] Character registered as agent #${characterAgentId}`,
    );
  ...
  async ensurePlayerRegistered(
    ...
    if (existingAgentId > 0) {
      console.log(
        `[RegistryClient] Player ${playerAddress} already registered as Agent #${existingAgentId}`,
      );
    ...
    // Register new player
    console.log(
      `[RegistryClient] Auto-registering new player: ${playerAddress}`,
    );
