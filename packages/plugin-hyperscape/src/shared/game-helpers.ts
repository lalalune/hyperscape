...
const AREAS: Record<
  string,
  AreaInfo & { check: (x: number, z: number) => boolean }
> = {
  spawn: {
    name: "Brookhaven",
    description:
      "A peaceful starter town with cobblestone paths. Bank and general store nearby.",
    safeZone: true,
  ...
  mistwood: {
    ...
    description:
      "A foggy forest with scattered oak trees. Goblin camps dot the landscape.",
  ...
  lakes: {
    ...
    description:
      "Interconnected lakes with fishing spots. Bandits control crossings.",
...
export function determineArea(position: [number, number, number]): AreaInfo {
  ...
  return {
    name: "The Wilderness",
    description: "Open terrain between major regions.",
  };
}
...
export interface CategorizedEntities {
  mobs: Array<
    Entity & { distance: number; direction: string; threat?: ThreatLevel }
  >;
...
export function categorizeEntities(
  ...
  for (const entity of entities) {
    ...
    if (
      ...
      entityAny.type === "mob" ||
      (entity.name &&
        /goblin|bandit|warrior|knight|skeleton|wolf|spider|rat/i.test(
          entity.name,
        ))
    ) {
      if (entityAny.alive !== false) {
        const threat = combatLevel
          ? assessThreat(entity.name || "unknown", combatLevel)
          : undefined;
      ...
      (entity.name &&
        /tree|rock|ore|fish/i.test(entity.name) &&
...
export function getPlayerStatus(player: PlayerEntity): PlayerStatus {
  ...
  return {
    ...
    health: {
      ...
      max: healthMax,
      percent:
        healthMax > 0 ? Math.round((healthCurrent / healthMax) * 100) : 100,
    },
    stamina: {
      current: staminaCurrent,
      max: staminaMax,
      percent:
        staminaMax > 0 ? Math.round((staminaCurrent / staminaMax) * 100) : 100,
    },
...
export function generateSceneDescription(
  ...
  if (categorized.mobs.length > 0) {
    const threats = categorized.mobs.filter(
      (m) => m.threat === "dangerous" || m.threat === "high",
    );
    const safe = categorized.mobs.filter(
      (m) => m.threat === "safe" || m.threat === "low" || !m.threat,
    );
    ...
    if (threats.length > 0) {
      ...
      for (const mob of threats.slice(0, 3)) {
        lines.push(
          `  • ${mob.name} [${(mob.threat || "unknown").toUpperCase()}] - ${Math.round(mob.distance)}m ${mob.direction}`,
        );
      }
    }
    ...
    if (safe.length > 0) {
      ...
      for (const mob of safe.slice(0, maxMobs - threats.length)) {
        lines.push(
          `  • ${mob.name} - ${Math.round(mob.distance)}m ${mob.direction}`,
        );
      }
    }
    lines.push("");
  ...
  if (categorized.resources.length > 0) {
    ...
    for (const res of categorized.resources.slice(0, maxResources)) {
      lines.push(
        `  • ${res.name} - ${Math.round(res.distance)}m ${res.direction}`,
      );
    }
    lines.push("");
  }
  ...
  if (includeStatus) {
    lines.push(
      `Health: ${status.health.percent}% | Stamina: ${status.stamina.percent}%`,
    );
    if (status.inCombat) {
  ...
  if (includeSuggestions) {
    ...
    if (status.health.percent < 30 && categorized.mobs.length > 0) {
      ...
      lines.push(
        `  • ${categorized.mobs.length} creature(s) nearby for combat`,
      );
