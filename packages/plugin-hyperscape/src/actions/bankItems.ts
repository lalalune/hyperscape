...
export const bankItemsAction: Action = {
  ...
  description:
    "Find and navigate to the nearest bank, then deposit items into it",
  ...
  handler: async (
    ...
    if (!service || !world || !player) {
      logger.error(
        "[BANK_ITEMS] Hyperscape service, world, or player not available",
      );
      if (callback) {
        await callback({
    ...
    try {
      ...
      const bankResult = await new Promise<
        BankDepositCompleteData | { success: boolean; error: string }
      >((resolve) => {
