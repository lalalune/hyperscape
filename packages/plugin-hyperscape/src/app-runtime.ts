...
export interface HyperscapeBridgeRuntimeLike {
  agentId?: string;
  character?: {
    name?: string;
...
export interface HyperscapeLaunchDiagnostic {
  code: string;
  severity: "info" | "warning" | "error";
  message: string;
}

export interface HyperscapeViewerAuthMessage {
...
async function resolveRuntimeWalletCandidate(
  ...
  const managedEvmAddress = readRuntimeSetting(runtime, MANAGED_EVM_ADDRESS_ENV_KEY);
...
function resolveHyperscapeApiBaseUrl(
  ...
  const runtimeUrl = readRuntimeSetting(runtime, "HYPERSCAPE_API_URL");
  if (runtimeUrl) {
...
async function authenticateHyperscapeWallet(
  ...
  const response = await fetch(
    new URL("/api/agents/wallet-auth", resolveHyperscapeApiBaseUrl(runtime)),
    {
      method: "POST",
      headers: {
...
export async function prepareHyperscapeAppLaunch(
  runtime: HyperscapeBridgeRuntimeLike | null,
  ...
  const authToken = readRuntimeSetting(runtime, "HYPERSCAPE_AUTH_TOKEN");
  const characterId = readRuntimeSetting(runtime, "HYPERSCAPE_CHARACTER_ID");
...
export function resolveHyperscapeViewerAuthMessage(
  runtime: HyperscapeBridgeRuntimeLike | null,
): HyperscapeViewerAuthMessage | null {
  const authToken = readRuntimeSetting(runtime, "HYPERSCAPE_AUTH_TOKEN");
  if (!authToken) {
    return null;
  }
...
export function isHyperscapeRuntimeReady(
  runtime: HyperscapeBridgeRuntimeLike | null,
): boolean {
  return Boolean(
    runtime &&
...
export async function ensureHyperscapeRuntimeReady(
  runtime: HyperscapeBridgeRuntimeLike | null,
): Promise<void> {
  if (!runtime) {
    return;
  }
  if (!isHyperscapeRuntimeReady(runtime)) {
    throw new Error(
      "Hyperscape service was not registered on the agent runtime.",
    );
  }
  if (typeof runtime.getServiceLoadPromise === "function") {
    await runtime.getServiceLoadPromise("hyperscapeService");
  }
}

export function collectHyperscapeLaunchDiagnostics(params: {
  requestedViewerAuth: boolean;
  runtime: HyperscapeBridgeRuntimeLike | null;
  sessionFound: boolean;
  viewerAuthMessage: HyperscapeViewerAuthMessage | null;
}): HyperscapeLaunchDiagnostic[] {
  const diagnostics: HyperscapeLaunchDiagnostic[] = [];
  const authToken = readRuntimeSetting(params.runtime, "HYPERSCAPE_AUTH_TOKEN");
  ...
  if (params.requestedViewerAuth && !params.viewerAuthMessage) {
    ...
    if (!authToken) {
      missing.push("HYPERSCAPE_AUTH_TOKEN");
    }
    if (!characterId) {
      missing.push("HYPERSCAPE_CHARACTER_ID");
    }
    diagnostics.push({
  ...
  if (params.runtime && !params.sessionFound && !isHyperscapeRuntimeReady(params.runtime)) {
    diagnostics.push({
      code: "hyperscape-runtime-bridge-inactive",
      severity: "warning",
      message:
        "The Hyperscape runtime bridge is not active in this agent, so the host cannot attach to a live in-world session yet.",
    });
  }
