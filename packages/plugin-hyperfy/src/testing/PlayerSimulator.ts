...
export class PlayerSimulator extends EventEmitter {
  ...
  async simulate(scenario: GameScenario): Promise<SimulationResult> {
    ...
    // Run simulation
    while (!this.gameState.checkWinCondition().gameEnded && 
    ...
    const player: SimulatedPlayer = {
      ...
      async analyzeBehavior(events: any[]): Promise<Map<string, number>> {
        ...
        for (const event of events) {
          ...
          suspicions.set(event.playerId, Math.min(1, currentSuspicion + increase));
        }
      ...
      async selectTarget(gameState: GameState, victims: any[]): Promise<any> {
        ...
        const scoredTargets = victims.map(victim => ({
          ...victim,
          score: (victim.isolated ? 2 : 1) * (1 / (1 + this.distance(this.position, victim.position)))
        }));
        ...
        return {
          ...
          reason: scoredTargets[0].isolated ? 'Target is isolated' : 'Target is nearby'
        };
      }
    };
  ...
  createMeetingSimulator() {
    return {
      async simulateDiscussion(participants: any[], context: any, duration: number) {
        const messages: any[] = [];
        const startTime = Date.now();
        
        // Generate initial reactions
        for (const participant of participants) {
          if (this.rng() > 0.3) { // 70% chance to speak initially
            messages.push({
        ...
        while (Date.now() - startTime < duration) {
          const speaker = participants[Math.floor(this.rng() * participants.length)];
          ...
          await new Promise(resolve => setTimeout(resolve, 1000 + this.rng() * 2000));
        }
  ...
  async replay(recording: GameRecording, options?: { speed: number }): Promise<SimulationResult> {
  ...
  private async initializeGame(scenario: GameScenario): Promise<void> {
    ...
    for (const player of players) {
      const behavior = player.role === PlayerRole.IMPOSTOR ? 
  ...
  private taskFocusedDecision(gameState: GameState): PlayerDecision {
    const uncompletedTasks = Array.from(gameState.tasks.values())
      .filter(task => !task.completedBy.has(this.id));
  ...
  private finalizeMetrics(): SimulationMetrics {
    return {
      ...
      meetingsCalled: this.events.filter(e => e.type === 'meeting_start').length,
      impostorsEjected: this.events.filter(e => e.type === 'ejection' && e.wasImpostor).length,
      avgKillsPerImpostor: 0, // Calculate from events
      sabotagesTriggered: this.events.filter(e => e.type === 'sabotage').length,
      sabotagesFixed: this.events.filter(e => e.type === 'sabotage_fixed').length,
  ...
  private countSurvivingCrewmates(): number {
    return Array.from(this.gameState.players.values())
      .filter(p => p.alive && p.role === PlayerRole.CREWMATE)
      .length;
  }
  ...
  private async executeDecision(playerId: string, decision: PlayerDecision): Promise<void> {
  ...
  private generateStatement(speaker: any, previousMessages: any[], context: any): string {
