...
export class HyperfyGameService {
  ...
  constructor(hyperfyService: HyperfyService) {
    this.hyperfyService = hyperfyService;
  }

  async movePlayer(playerId: string, position: { x: number; y: number; z: number }): Promise<void> {
    try {
      ...
      elizaLogger.info(`Player ${playerId} moved to ${position.x}, ${position.y}, ${position.z}`);
    } catch (error) {
  ...
  getWorld() {
    return this.hyperfyService.getWorld();
  }

  isConnected(): boolean {
    return this.hyperfyService.isConnected();
  }
} 