...
describe('Plugin Configuration', () => {
  it('should have correct plugin metadata', () => {
    ...
    expect(hyperfyPlugin.description).toBe(
      'Integrates ElizaOS agents with Hyperfy worlds'
    )
  ...
  it('should have a valid config', () => {
    ...
    if (hyperfyPlugin.config) {
      ...
      expect(Object.keys(hyperfyPlugin.config)).toContain(
        'DEFAULT_HYPERFY_WS_URL'
      )
    }
  })
})
...
describe('HyperfyService', () => {
  ...
  it('should throw an error when stopping a non-existent service', async () => {
    ...
    await expect(HyperfyService.stop(runtime as any)).rejects.toThrow(
      'Hyperfy service not found'
    )
