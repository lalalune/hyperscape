...
describe('Real Agent Integration', () => {
  ...
  describe('Agent Decision Making', () => {
    it('should make role-appropriate decisions as crewmate', async () => {
      ...
      const taskDecisions = decisions.filter(d => 
    ...
    it('should exhibit impostor behavior when assigned impostor role', async () => {
      ...
      const killConsiderations = decisions.filter(d => 
        d.action === 'move_to_player' || d.action === 'kill'
      );
      expect(killConsiderations.length).toBeGreaterThan(0);
      
      // Should fake tasks
      const fakeTaskDecisions = decisions.filter(d => 
        d.action === 'move_to_task' && d.reason?.includes('fake')
      );
  ...
  describe('Performance Under Load', () => {
    it('should handle multiple concurrent games', async () => {
      ...
      await Promise.all(
        agents.map(agent => agent.makeDecision(gameState))
      );
    ...
    it('should maintain performance with rapid state changes', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        // Modify game state
        if (i % 10 === 0) {
          gameState.phase = gameState.phase === 'playing' ? 'meeting' : 'playing';
        }
