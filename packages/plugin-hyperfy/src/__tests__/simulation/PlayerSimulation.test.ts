...
describe('Player Simulation Tests', () => {
  ...
  describe('Complex Scenarios', () => {
    ...
    it('should simulate sabotage scenarios', async () => {
      ...
      const sabotageKills = result.events.filter(
        e =>
          e.type === 'kill' &&
          e.timestamp -
            result.events.find(s => s.type === 'sabotage')!.timestamp <
            10000
      )
    ...
    it('should handle AFK players', async () => {
      ...
      const afkTasks = result.events.filter(
        e => e.type === 'task_complete' && ['p2', 'p4'].includes(e.playerId)
      )
  ...
  describe('Player Behaviors', () => {
    it('should simulate task-focused crewmates', async () => {
      ...
      const taskActions = decisions.filter(
  ...
  describe('Meeting Dynamics', () => {
    it('should simulate realistic discussions', async () => {
      ...
      const analyticalMessages = discussion.messages.filter(
        m => m.playerId === 'p1'
      )
      expect(analyticalMessages.some(m => m.text.includes('?'))).toBe(true)

      // Impostor should deflect
      const impostorMessages = discussion.messages.filter(
        m => m.playerId === 'p2'
      )
      expect(
        impostorMessages.some(
          m =>
      ...
      const voters = [
        {
          id: 'p1',
          role: PlayerRole.CREWMATE,
          suspicions: new Map([['p2', 0.8]]),
        },
        {
          id: 'p2',
          role: PlayerRole.IMPOSTOR,
          suspicions: new Map([['p3', 0.6]]),
        },
        {
          id: 'p3',
          role: PlayerRole.CREWMATE,
          suspicions: new Map([['p2', 0.7]]),
        },
        {
          id: 'p4',
          role: PlayerRole.CREWMATE,
          suspicions: new Map([['p2', 0.5]]),
        },
      ]
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle simultaneous kills', async () => {
      ...
      const simultaneousKills = result.events.filter(
        e => e.type === 'kill' && Math.abs(e.timestamp - 30000) < 100
      )
