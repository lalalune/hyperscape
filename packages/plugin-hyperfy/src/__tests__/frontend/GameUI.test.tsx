...
describe('GameUI Component', () => {
  ...
  describe('Role Display', () => {
    it('should display crewmate role correctly', () => {
      render(
        <GameProvider>
          <GameUI {...defaultProps} role={PlayerRole.CREWMATE} />
        </GameProvider>
      )
    ...
    it('should display impostor role correctly', () => {
      render(
        <GameProvider>
          <GameUI {...defaultProps} role={PlayerRole.IMPOSTOR} />
        </GameProvider>
      )
  ...
  describe('Meeting UI', () => {
    ...
    it('should enable voting after discussion', () => {
      ...
      rerender(
        <GameProvider>
          <GameUI {...defaultProps} gamePhase="voting" players={mockPlayers} />
        </GameProvider>
      )
    ...
    it('should not allow voting for dead players', () => {
      ...
      expect(screen.getByText('Player 3').parentElement).toHaveClass(
        'opacity-50'
      )

      const deadPlayerContainer = screen
        .getByText('Player 3')
        .closest('[data-testid="player-vote-option"]')
      expect(
        deadPlayerContainer?.querySelector('button')
      ).not.toBeInTheDocument()
    })
  ...
  describe('Chat System', () => {
    it('should display chat messages', () => {
      ...
      render(
        <GameProvider>
          <GameUI {...defaultProps} messages={messages} />
        </GameProvider>
      )

      expect(screen.getByText('Player 1: I saw red vent!')).toBeInTheDocument()
      expect(
        screen.getByText('Player 2: I was in electrical')
      ).toBeInTheDocument()
    })
    ...
    it('should allow sending messages during meetings', async () => {
      ...
      render(
        <GameProvider>
          <GameUI {...defaultProps} gamePhase="meeting" />
        </GameProvider>
      )
    ...
    it('should disable chat during gameplay', () => {
      render(
        <GameProvider>
          <GameUI {...defaultProps} gamePhase="playing" />
        </GameProvider>
      )
  ...
  describe('Player List', () => {
    ...
    it('should display player list in lobby', () => {
      render(
        <GameProvider>
          <GameUI {...defaultProps} gamePhase="lobby" players={mockPlayers} />
        </GameProvider>
      )
  ...
  describe('Accessibility', () => {
    it('should have proper ARIA labels', () => {
      ...
      expect(
        screen.getByRole('button', { name: /kill player/i })
      ).toHaveAttribute('aria-label')
      expect(
        screen.getByRole('region', { name: /game status/i })
      ).toBeInTheDocument()
    })
    ...
    it('should be keyboard navigable', async () => {
      ...
      await user.tab()
      expect(
        screen.getByRole('button', { name: /vote player 1/i })
      ).toHaveFocus()

      await user.tab()
      expect(
        screen.getByRole('button', { name: /vote player 2/i })
      ).toHaveFocus()
    })
    ...
    it('should announce important updates to screen readers', () => {
      ...
      rerender(
        <GameProvider>
          <GameUI {...defaultProps} announcement="Body reported by Player 2!" />
        </GameProvider>
      )
