...
export class PlayerMovementSystem extends EventEmitter {
  private world: any;
  private movingPlayers: Map<string, { target: Vector3; path?: Vector3[] }> = new Map();
  ...
  findPath(start: Vector3, end: Vector3): Vector3[] | null {
    ...
    while (openSet.length > 0) {
      ...
      for (const neighborPos of neighbors) {
        ...
        const g = current.g + MathUtils.distance2D(current.position, neighborPos);
  ...
  private handleCollision(player: any, movement: { target: Vector3; path?: Vector3[] }): void {
