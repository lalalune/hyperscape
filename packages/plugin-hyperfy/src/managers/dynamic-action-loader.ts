...
export interface HyperfyActionDescriptor {
  ...
  examples: string[];
  category: 'combat' | 'inventory' | 'skills' | 'quest' | 'social' | 'movement' | 'other';
...
export class DynamicActionLoader {
   ...
   */
  async discoverActions(world: HyperfyWorld): Promise<HyperfyActionDescriptor[]> {
    try {
      ...
      if (world.actions?.getAvailableActions) {
        ...
        logger.info(`[DynamicActionLoader] Found ${actions.length} actions from world`);
        return actions;
      }
      ...
      if (world.entities?.items) {
        world.entities.items.forEach((entity: any) => {
          if (entity.components?.find((c: any) => c.type === 'action-provider')) {
            const actionComponent = entity.components.find((c: any) => c.type === 'action-provider');
      ...
      logger.info(`[DynamicActionLoader] Found ${actionProviders.length} actions from entity scan`);
   ...
   */
  async registerAction(descriptor: HyperfyActionDescriptor, runtime: IAgentRuntime): Promise<void> {
    ...
    if (runtime.registerAction) {
      ...
      logger.info(`[DynamicActionLoader] Successfully registered action: ${descriptor.name}`);
    } else {
      ...
      if (runtime.actions && Array.isArray(runtime.actions)) {
        ...
        logger.info(`[DynamicActionLoader] Added action to runtime: ${descriptor.name}`);
      } else {
        logger.warn(`[DynamicActionLoader] Unable to register action - no registration method available`);
      }
    }
  }
   ...
   */
  async unregisterAction(actionName: string, runtime: IAgentRuntime): Promise<void> {
  ...
  private createDynamicHandler(descriptor: HyperfyActionDescriptor) {
    return async (
      ...
      const params = await this.extractParameters(descriptor, message, state, runtime);
      ...
      try {
        ...
        const responseText = await this.generateResponse(descriptor, params, result, runtime, state);
        ...
        logger.error(`[DynamicAction] Error executing ${descriptor.name}:`, error);
  ...
  private generateSimiles(descriptor: HyperfyActionDescriptor): string[] {
    ...
    if (words.length > 1) {
      ...
      similes.push(words.map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(''));
    }
