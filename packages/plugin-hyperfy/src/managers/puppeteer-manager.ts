// @ts-nocheck - Complex Puppeteer integration with many dynamic types
import path from 'path'
import fs from 'fs'
import { promises as fsPromises } from 'fs'
import puppeteer from 'puppeteer'
import { IAgentRuntime, ModelType } from '../types/eliza-mock'
...
export class PuppeteerManager {
  ...
  public getPage(): puppeteer.Page | undefined {
    return this.page
  }
    ...
    if (!this.initPromise) {
      this.initPromise = (async () => {
        try {
          ...
          console.warn(
            '[PuppeteerManager] Failed to initialize Puppeteer:',
            error
          )
          console.warn(
            '[PuppeteerManager] Screenshot functionality will be disabled'
          )
  ...
  public async snapshotFacingDirection(
    ...
    if (!this.browser || !this.page) {
      console.warn(
        '[PuppeteerManager] Puppeteer not available, skipping screenshot'
      )
      return ''
    }
