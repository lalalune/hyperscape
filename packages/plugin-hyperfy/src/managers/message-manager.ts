...
export class MessageManager {
  ...
  async handleMessage(msg: any): Promise<void> {
    ...
    await agentActivityLock.run(async () => {
      ...
      console.info(
        `[Chat Received] From: ${senderName}, ID: ${msg.id}, Body: "${messageBody}"`
      )
      ...
      if (msg.fromId && msg.fromId !== agentPlayerId) {
        ...
        const hyperfyWorldId = createUniqueUuid(
          this.runtime,
          'hyperfy-world'
        ) as UUID
        ...
        console.debug(
          `[Hyperfy Chat] Creating entity connection for: ${entityId}`
        )
        ...
        try {
          ...
          console.error(
            `[Hyperfy Chat] Error checking/creating entity: ${error}`
          )
        }

        // Emit the MESSAGE_RECEIVED event to trigger the message handler
        console.info(
          `[Hyperfy Chat] Emitting MESSAGE_RECEIVED event for message: ${messageId}`
        )
        ...
        console.info(
          `[Hyperfy Chat] Successfully emitted event for message: ${messageId}`
        )
      }
    })
  }
  ...
  async sendMessage(text: string): Promise<void> {
    const service = this.runtime.getService<HyperfyService>(
    ...
    const messageStrings = messages
      ...
      .map((message: Memory) => {
        ...
        const entity = entities.find(
          (e: Entity) => e.id === message.entityId
        ) as any
        const formattedName = (() => {
          try {
        ...
        const formattedId = entity
          ? entity.data
            ? JSON.parse(entity.data).hyperfy.id
            : ''
          : ''
  ...
  async getRecentMessages(roomId: UUID, count = 20) {
    ...
    const lastResponseText =
      recentMessagesData
        ...
        .sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))[0]?.content
        .text || null
