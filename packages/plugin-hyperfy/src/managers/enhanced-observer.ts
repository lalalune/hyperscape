...
export class EnhancedObserver {
  ...
  private async openObservationWindow(): Promise<void> {
    ...
    try {
      ...
      logger.error(
        '[EnhancedObserver] Error opening observation window:',
        error
      )
    }
  }
    ...
    try {
      ...
      if (this.recording && this.puppeteerManager) {
        const shouldScreenshot =
          this.recording.screenshots.length === 0 ||
          Date.now() -
            this.recording.screenshots[
              this.recording.screenshots.length - 1
            ].timestamp.getTime() >
            10000
