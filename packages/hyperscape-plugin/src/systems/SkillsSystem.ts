 ...
 *
 * Implements tile-based-MMORPG-style skill progression system with experience points and levels.
 *
 * **Skills Managed:**
 ...
 *
 * **XP Calculation:**
 * Uses tile-based MMORPG XP table formula:
...
export class SkillsSystem extends SystemBase {
  ...
  public getCombatLevel(stats: StatsComponent): number {
    // tile-based MMORPG combat level formula
  ...
  private handleLevelUp(
    ...
    // Prayer level = max prayer points (tile-based-MMORPG-accurate)
    if (skill === Skill.PRAYER) {
      ...
      if (prayerSystem?.setMaxPrayerPoints) {
        ...
        // Also restore prayer points to new max (tile-based MMORPG behavior on level-up)
  ...
  private calculateMaxHitpoints(level: number): number {
    // tile-based MMORPG formula: 10 + level
    return 10 + level;
  }
  ...
  private handleCombatKill(data: {
    ...
    // tile-based MMORPG Formula: 4 XP per damage for combat skills, 1.33 XP per damage for Hitpoints
    ...
    switch (attackStyle) {
      ...
      case "controlled": {
        // Tile-based MMORPG: Controlled gives 1.33 XP per damage to each of 4 skills
    ...
    // ALWAYS grant Hitpoints XP (Constitution) for combat - tile-based MMORPG standard
    this.emitTypedEvent(EventType.SKILLS_XP_GAINED, {
      playerId: attackerId,
      skill: Skill.CONSTITUTION,
      amount: hitpointsXP,
