...
tile-based-MMORPG-accurate resource gathering system for woodcutting, mining, and fishing.

## Overview

The gathering system implements authentic tile-based MMORPG mechanics including:
...
├── DropRoller.ts         # tile-based MMORPG drop mechanics & fish priority rolling
├── ToolUtils.ts          # Tool validation & category mapping
├── SuccessRateCalculator.ts  # tile-based MMORPG LERP formula implementation
└── README.md             # This file
```
...
| `DropRoller.ts` | Roll drops using tile-based MMORPG chance distribution, priority fish rolling |
| `ToolUtils.ts` | Map item IDs to tool categories, validate tool requirements |
| `SuccessRateCalculator.ts` | Calculate success rates using tile-based MMORPG LERP formula |
...
| `FaceDirectionManager.ts` | tile-based-MMORPG-accurate deferred face direction at tick end |

## Tile-based MMORPG mechanics Implemented
...
### Tick System
- All gathering runs on 600ms ticks (tile-based MMORPG standard)
...
- **600ms rate limit**: Silently drops requests faster than 1 tick (matches the tile-based MMORPG genre)
