...
export async function handleEnterWorld(
  ...
  const agentManager = world.getSystem(
    "agent-manager",
  ) as unknown as IAgentManager | null;
  const agentRuntimeLookup = world.getSystem(
    "agent-runtime-lookup",
  ) as unknown as IAgentRuntimeLookup | null;
  const isEmbeddedAgent = !!(
    characterId && agentManager?.hasAgent(characterId)
  );
  const isModelAgent = !!(
    characterId && agentRuntimeLookup?.getAgentRuntimeByCharacterId(characterId)
  );
      ...
      if (databaseSystem) {
        ...
        if (!characterData) {
          ...
          if (db) {
            ...
            // Drizzle types are not available in shared; assert the shape
            // we need at the call site.
            type CharQueryShape = {
              query: {
                characters: {
                  findFirst(opts: {
            ...
            const typedDb = db as CharQueryShape;
            const directLookup = await typedDb.query.characters.findFirst({
  ...
  if (socket.player) {
    ...
    const dbSys = world.getSystem?.("database") as IDatabaseSystem | undefined;
