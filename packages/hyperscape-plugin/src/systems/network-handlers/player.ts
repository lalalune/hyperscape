 ...
 *
 * Database access:
 * - Uses `world.getSystem("database") as IDatabaseSystem` for character repo
 *   access — no direct DB / Drizzle imports so this handler can live in shared.
 *
...
export function handleChangePlayerName(
  ...
  // Look up DatabaseSystem via world — no direct DB dependency in shared
  const dbSystem = world.getSystem("database") as IDatabaseSystem | undefined;
  const characterRepo = dbSystem?.getCharacterRepository?.();
  if (!characterRepo) {
    sendPlayerError(socket, "Server error: database unavailable");
    return;
  }

  // Update database asynchronously via the narrow interface
  characterRepo
