...
> **The Complete Technical Reference for Hyperia's Tile-based-MMORPG-accurate Combat System**
...
10. [tile-based MMORPG Combat Mechanics](#10-osrs-combat-mechanics)
...
|-----------|---------------|
| **tile-based MMORPG Accuracy** | Tick-based timing (600ms), authentic formulas, PID shuffle, tolerance timer |
...
│   ├── RangeSystem.ts               # tile-based MMORPG range calculations (300 lines)
...
│   ├── PidManager.ts                # tile-based MMORPG PID system (392 lines)
│   └── handlers/
...
│   └── GroundItemSystem.ts          # tile-based-MMORPG-style ground items (633 lines)
├── systems/shared/character/
│   └── HealthRegenSystem.ts         # tile-based MMORPG passive health regen (230 lines)
├── systems/client/
...
└── utils/
    ...
    └── game/
        ├── CombatLevelCalculator.ts # tile-based MMORPG combat level formula (156 lines)
        ├── CombatCalculations.ts    # Damage/accuracy formulas (468 lines)
        ├── CombatValidation.ts      # Input validation utilities (223 lines)
        ├── HitDelayCalculator.ts    # tile-based MMORPG hit delay/projectiles (345 lines)
...
The `PlayerCombatStateManager.ts` (406 lines) manages player-specific combat state with tile-based-MMORPG-accurate mechanics.
...
class PlayerCombatStateManager {
  ...
  // Auto-retaliate (enabled by default in the tile-based MMORPG genre)
  ...
  // tile-based MMORPG auto-retaliate timing: ceil(attackSpeed / 2) + 1 ticks
...
**Tile-based-MMORPG-accurate Features:**
...
### 3.7 PidManager (tile-based MMORPG PID Shuffle)

Ensures fair PvP combat priority using tile-based-MMORPG-accurate PID system.
...
tile-based-MMORPG-accurate tolerance timer that stops aggression after 10 minutes in a region.
...
tile-based-MMORPG-accurate combat level formula for aggression checks.
...
The `AggroManager.ts` (306 lines) handles target acquisition with tile-based-MMORPG-accurate random selection.
...
class AggroManager {
  ...
  // tile-based-MMORPG-accurate: Random selection from ALL valid candidates
...
### 5.1 Three tile-based MMORPG Range Types

```
┌─────────────────────────────────────────────────────────────────┐
│                    tile-based MMORPG RANGE TYPES                              │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
...
```
┌─────────────────────────────────────────────────────────────────┐
│                    tile-based MMORPG MELEE RANGE RULES                        │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
...
│           ▼                                                      │
│  ┌──────────────────┐                                           │
│  │ Spawn Gravestone │ ──► AFTER respawn (tile-based-MMORPG-style)       │
│  │ (Safe Areas)     │                                           │
│  └──────────────────┘                                           │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
...
**Tile-based-MMORPG-accurate Timing:**
...
The `RespawnManager.ts` (251 lines) manages mob respawn locations with tile-based-MMORPG-accurate tick-based timing.
...
**tile-based MMORPG-Style Behavior:**
...
The `GroundItemSystem.ts` (633 lines) manages tile-based-MMORPG-style ground items with tile-based piling.
...
The `HitDelayCalculator.ts` (345 lines) calculates tile-based-MMORPG-accurate hit delays and manages projectile tracking.
...
class CombatStateManager {
  ...
  // First-attack timing (tile-based-MMORPG-accurate)
  ...
  canAttack(currentTick: number): boolean;
  performAttack(targetId: string, currentTick: number): boolean;
  onReceiveAttack(currentTick: number): void;  // tile-based MMORPG retaliation timing
}
```

### 8.5 HealthRegenSystem

The `HealthRegenSystem.ts` (230 lines) handles tile-based-MMORPG-style passive health regeneration.
...
The `DamageSplatSystem.ts` (238 lines) creates tile-based-MMORPG-style damage splats.
...
## 10. tile-based MMORPG Combat Mechanics

### 10.1 Tick-Based Timing

```
┌─────────────────────────────────────────────────────────────────┐
│                    tile-based MMORPG TICK SYSTEM                              │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
...
│  │ (2000 ticks)     │             (tile-based-MMORPG-accurate)               │
│  └────────┬─────────┘                                           │
│           │ NO                                                   │
│           ▼                                                      │
│  ┌──────────────────┐                                           │
...
### 12.1 tile-based MMORPG Accuracy Formula
...
### 12.2 tile-based MMORPG Max Hit Formula
...
```typescript
// Combat Style Bonuses (tile-based-MMORPG-accurate)
...
│  • Uses tile-based MMORPG formula                                             │
...
This combat system represents a **production-ready, tile-based-MMORPG-accurate** implementation with:
...
| **tile-based MMORPG Accuracy** | Full (tick system, formulas, tolerance timer, PID shuffle) |
...
- ✅ tile-based MMORPG tick-based timing (600ms)
...
- ✅ Ground items with tile-based MMORPG tile piling
- ✅ tile-based MMORPG health regeneration (17-tick combat cooldown)
- ✅ Animation-damage-hitsplat synchronization
- ✅ Hit delay by attack type (melee/ranged/magic)
- ✅ tile-based-MMORPG-style damage splats (red/blue)
- ✅ XP drop notifications with skill icons
- ✅ Auto-retaliate with tile-based MMORPG timing
