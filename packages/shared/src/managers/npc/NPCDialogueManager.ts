...
export class NPCDialogueManager {
  ...
  startDialogue(playerId: string, npcId: string): void {
    ...
    // Check if dialogue exists for this NPC
    const npcDialogue = this.dialogues.get(npcId);
    if (!npcDialogue || npcDialogue.size === 0) return;
    ...
    // Emit event
    this.world.emit('dialogue:start', {
      playerId,
      npcId
    });
  }
