...
// Helper function to create default prayer component
function createDefaultPrayerComponent(): PrayerComponent {
  return {
...
export class DamageCalculator {
  ...
  private getProtectionPrayerMultiplier(target: StatsComponent, attackType: AttackType): number {
    ...
    const prayers = target.activePrayers || createDefaultPrayerComponent();
  ...
  private getDefensiveDamageReduction(target: StatsComponent, attackType: AttackType): number {
    ...
    switch (attackType) {
      case AttackType.MELEE:
        ...
        defenseBonus = (
          (target.combatBonuses?.defenseStab ?? 0) +
          (target.combatBonuses?.defenseSlash ?? 0) +
          (target.combatBonuses?.defenseCrush ?? 0)
        ) / 3;
        break;
      case AttackType.RANGED:
        defenseBonus = target.combatBonuses?.defenseRanged ?? 0;
        break;
      case AttackType.MAGIC:
        defenseBonus = target.combatBonuses?.defenseMagic ?? 0;
        break;
    }
  ...
  private getSpecialDefensiveReduction(target: StatsComponent): number {
    ...
    const shield = equipment.shield?.item;
    
    // Elysian spirit shield effect (25% chance to reduce damage by 25%)
    if (shield?.name === 'Elysian spirit shield') {
      if (Math.random() < 0.25) {
    ...
    const prayerPoints = typeof target.prayer === 'object' ? target.prayer?.points : 0;
    if (shield?.name === 'Divine spirit shield' && prayerPoints && prayerPoints > 0) {
  ...
  private calculateMeleeMaxHit(attacker: StatsComponent, style: CombatStyle): number {
    ...
    const strengthBonus = attacker.combatBonuses?.meleeStrength ?? 0;
    
    // RuneScape formula: 0.5 + effectiveStrength * (strengthBonus + 64) / 640
    let maxHit = 0.5 + (effectiveStrength * ((strengthBonus ?? 0) + 64)) / 640;
  ...
  private calculateRangedMaxHit(attacker: StatsComponent, style: CombatStyle): number {
    ...
    const rangedStrength = attacker.combatBonuses?.rangedStrength ?? 0;
    
    // Similar formula to melee
    let maxHit = 0.5 + (effectiveRanged * ((rangedStrength ?? 0) + 64)) / 640;
  ...
  private calculateMagicMaxHit(attacker: StatsComponent): number {
    const magicLevel = typeof attacker.magic === 'object' ? attacker.magic?.level || 1 : attacker.magic || 1;
    const magicDamage = attacker.combatBonuses?.magicDamage ?? 0;
    
    // Get spell damage from equipped spell
    const baseSpellDamage = this.getEquippedSpellDamage(attacker);
    
    // Apply magic damage bonus
    let maxHit = baseSpellDamage * (1 + (magicDamage ?? 0) / 100);
  ...
  private getMeleePrayerBonus(attacker: StatsComponent): number {
    const prayers = attacker.activePrayers || createDefaultPrayerComponent();
  ...
  private getMeleeOtherBonuses(attacker: StatsComponent): number {
    ...
    const head = equipment.helmet?.item;
    const weapon = equipment.weapon?.item;
    const amulet = equipment.amulet?.item;
    
    // Slayer helm on task
    if (head?.name?.includes('Slayer helm') && effects?.onSlayerTask) {
      multiplier *= 1.1667; // 16.67% damage bonus
    }
    
    // Berserker necklace with obsidian weapons
    if (amulet?.name === 'Berserker necklace' &&
        weapon?.name?.includes('Obsidian')) {
      multiplier *= 1.20; // 20% damage bonus
    }
    
    // Dragon hunter lance vs dragons
    if (weapon?.name === 'Dragon hunter lance' && effects?.targetIsDragon) {
      multiplier *= 1.20; // 20% damage bonus
    }
  ...
  private getRangedOtherBonuses(attacker: StatsComponent): number {
    ...
    if (weapon?.name === 'Twisted bow' && effects?.targetMagicLevel) {
   ...
   */
  private hasVoidMeleeSet(equipment: EquipmentComponent): boolean {
    const head = equipment.helmet?.item;
    const body = equipment.body?.item;
    const legs = equipment.legs?.item;
    const gloves = equipment.gloves?.item;
    
    return head?.name === 'Void melee helm' &&
           body?.name === 'Void knight top' &&
           legs?.name === 'Void knight robe' &&
           gloves?.name === 'Void knight gloves';
  }
  
  /**
   * Check if player has void ranged set
   */
  private hasVoidRangedSet(equipment: EquipmentComponent): boolean {
    const head = equipment.helmet?.item;
    const body = equipment.body?.item;
    const legs = equipment.legs?.item;
    const gloves = equipment.gloves?.item;
    
    return head?.name === 'Void ranger helm' &&
           body?.name === 'Void knight top' &&
           legs?.name === 'Void knight robe' &&
           gloves?.name === 'Void knight gloves';
  }
   ...
   */
  private hasEliteVoidRangedSet(equipment: EquipmentComponent): boolean {
    const head = equipment.helmet?.item;
    const body = equipment.body?.item;
    const legs = equipment.legs?.item;
    const gloves = equipment.gloves?.item;
  ...
  private getEffectiveStrengthLevel(attacker: StatsComponent, style: CombatStyle): number {
    ...
    const strengthLevel = typeof attacker.strength === 'object' ? attacker.strength?.level || 1 : attacker.strength || 1;
    return strengthLevel + styleBonus + 8;
  }
  ...
  private getEffectiveRangedLevel(attacker: StatsComponent, _style: CombatStyle): number {
    ...
    // Effective level = level + style bonus + 8
    const rangedLevel = typeof attacker.ranged === 'object' ? attacker.ranged?.level || 1 : attacker.ranged || 1;
    return rangedLevel + styleBonus + 8;
  }
} 