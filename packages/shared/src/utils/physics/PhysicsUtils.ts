...
export function installThreeJSExtensions(): void {
  ...
  if (!THREE.Quaternion.prototype.toPxTransform) {
    THREE.Quaternion.prototype.toPxTransform = function (
 ...
 */
export function createCpuDispatcher(
  physx: PhysXModule,
  threads: number,
 ...
 */
export function getActorsFromHeader(
  header: PxContactPairHeader,
): [PxActor, PxActor] {
  const h = header as PxContactPairHeader & {
    get_actors(index: number): PxActor;
  };
...
export function isKinematic(physx: PhysXModule, actor: PxActor): boolean {
  const flags = (
    actor as PxActor & { getRigidBodyFlags(): { isSet(flag: number): boolean } }
  ).getRigidBodyFlags();
