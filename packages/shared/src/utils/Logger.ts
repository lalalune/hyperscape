...
class LoggerImpl {
  ...
  private systemStats = new Map<
    string,
    { errors: number; warnings: number; messages: number }
  >();
  ...
  public getTestLogs(count: number = 100): LogEntry[] {
    return this.logs.filter((log) => log.level === LogLevel.TEST).slice(-count);
  }
