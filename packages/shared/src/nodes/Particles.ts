...
export class Particles extends Node {
  ...
  _direction?: number;
  _rate?: number;
  _bursts?: Array<{ time: number; count: number }>;
  _duration?: number;
  _loop?: boolean;
  _max: number = defaults.max;
  _timescale?: number;
  _life?: string;
  _speed?: string;
  _size?: string;
  ...
  _emissive?: string;
  _image?: string;
  _spritesheet?: [number, number, number, number] | null;
  _blending?: string;
  _lit?: boolean;
  _billboard?: string;
  _space?: string;
  _force?: [number, number, number] | null;
  _velocityLinear?: [number, number, number] | null;
  _velocityOrbital?: [number, number, number] | null;
  ...
  constructor(data: ParticlesData = {}) {
    ...
    this.loop = data.loop
    this.max = data.max ?? defaults.max
