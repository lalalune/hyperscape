...
import type { UIImageData, UIYogaNodeContext as YogaNode, UIContext as UINode, UIImageNodeContext as UIImageNode, UIBoxNodeContext as UIBoxNode } from '../types/nodes'
...
export class UIImage extends Node {
  ...
  // Other properties
  private yogaNode: YogaNode | null = null
  private ui: UINode | null = null
  private img: UIImageNode | null = null
  private box: UIBoxNode | null = null
  ...
  constructor(data: UIImageData = {}) {
    ...
    this.width = typeof data.width === 'string' ? parseFloat(data.width) || null : (data.width ?? defaults.width)
    this.height = typeof data.height === 'string' ? parseFloat(data.height) || null : (data.height ?? defaults.height)
    ...
    this.objectFit = data.objectFit ?? defaults.objectFit
  ...
  draw(ctx: CanvasRenderingContext2D, offsetLeft: number, offsetTop: number): void {
  ...
  mount() {
    ...
    this.ui = (this.parent as { ui?: UINode })?.ui || null
  ...
  unmount() {
    ...
    if (this.yogaNode) {
      if (this.parent) {
        (this.parent as { yogaNode?: YogaNode }).yogaNode?.removeChild(this.yogaNode)
      }
      this.yogaNode.free()
      this.yogaNode = null
      this.box = null
