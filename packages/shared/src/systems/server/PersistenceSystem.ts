...
export class PersistenceSystem extends SystemBase {
  ...
  async init(): Promise<void> {
    ...
    this.databaseSystem =
      getSystem<DatabaseSystem>(this.world, "database") || undefined;
    ...
    this.terrainSystem =
      getSystem<TerrainSystem>(this.world, "terrain") || undefined;
    if (!this.terrainSystem) {
      // This is expected in test environments without terrain, so use debug level
      this.logger.debug(
        "TerrainSystem not found - chunk persistence will be limited",
      );
    }
    ...
    this.subscribe(EventType.PLAYER_JOINED, (data) =>
      this.onPlayerEnter(
        data as { playerId: string; userId?: string; playerToken?: string },
      ),
    );
    this.subscribe(EventType.PLAYER_LEFT, (data) =>
      this.onPlayerLeave(data as { playerId: string }),
    );
    this.subscribe(
      EventType.CHUNK_LOADED,
      (data: { chunkId: string; chunkData: WorldChunk }) => {
    ...
    this.subscribe(
      EventType.PERSISTENCE_SAVE,
      (data: { playerId: string; data?: Record<string, unknown> }) => {
  ...
  start(): void {
    ...
    this.logger.info(
      "Persistence services started - using frame-based updates",
    );
  }
  ...
  // Event Handlers
  private async onPlayerEnter(event: {
    playerId: string;
    userId?: string;
    playerToken?: string;
  }): Promise<void> {
  ...
  private async onPlayerLeave(event: {
    playerId: string;
    userId?: string;
    sessionId?: string;
    reason?: string;
    ...
    const playerSession = activeSessions.find(
      (s) => s.playerId === characterId,
    );

    if (playerSession) {
      await this.databaseSystem.endPlayerSessionAsync(
        playerSession.id,
  ...
  private async onChunkLoaded(event: {
    chunkX: number;
    chunkZ: number;
  }): Promise<void> {
    if (!this.databaseSystem) return;
  ...
  private async onChunkUnloaded(event: {
    chunkX: number;
    chunkZ: number;
  }): Promise<void> {
    if (!this.databaseSystem) return;
    ...
    if (this.terrainSystem && this.databaseSystem) {
      ...
      for (const chunk of activeChunks) {
        ...
        const chunkData: WorldChunkData = {
          ...
          lastActive: chunk.lastActivity
            ? chunk.lastActivity.getTime()
            : Date.now(),
  ...
  private async performChunkCleanup(): Promise<void> {
    ...
    const inactiveChunks = this.databaseSystem.getInactiveChunks(
      this.CHUNK_INACTIVE_TIME / 60000,
    ); // Convert to minutes
    ...
    if (inactiveChunks.length > 0) {
      this.logger.info(
        `🧹 Chunk cleanup: ${inactiveChunks.length} inactive chunks processed`,
      );
    }
  }
  ...
  private async performMaintenance(): Promise<void> {
    ...
    try {
      ...
      const oldSessionsDeleted =
        await this.databaseSystem.cleanupOldSessionsAsync(7);

      // Clean up old chunk activity records (30+ days old)
      const oldActivityDeleted =
        await this.databaseSystem.cleanupOldChunkActivityAsync(30);
