...
export class InterpolationEngine {
  ...
  private getOldestSnapshotIndex(state: InterpolationState): number {
  ...
  private getLatestSnapshotIndex(state: InterpolationState): number {
    if (state.snapshotCount === 0) {
      return 0;
    }
  ...
  private getSnapshotAt(
    ...
    const oldestIndex = this.getOldestSnapshotIndex(state);
  ...
  private interpolateEntityPosition(
    ...
    if (state.snapshotCount < 2) {
      if (state.snapshotCount === 1) {
        const snapshot = state.snapshots[this.getLatestSnapshotIndex(state)];
    ...
    for (let i = 0; i < state.snapshotCount - 1; i++) {
      const curr = this.getSnapshotAt(state, i);
      const next = this.getSnapshotAt(state, i + 1);
    ...
    if (older && newer) {
      ...
      if (timeSinceUpdate < this.extrapolationLimit) {
        const last = state.snapshots[this.getLatestSnapshotIndex(state)];
        state.tempPosition.set(
          last.position[0],
          last.position[1],
          last.position[2],
