/**
 * AtlasedRockPlantImpostorManager - Unified Atlas for Rocks and Plants
 *
 * Combines rock and plant impostor atlases into a single mega-atlas with 16 slots.
 * All rock/plant impostors share ONE InstancedMesh, reducing draw calls from
 * N (one per preset) to 1.
 *
 * If more than 16 unique rock/plant presets are used, excess presets won't render
 * as impostors (they'll be culled at impostor distance).
 *
 * @module AtlasedRockPlantImpostorManager
 */
...
/**
 * TEMPORARY: Disable all rock/plant atlased impostors.
 * When true, the manager skips all mesh creation and instance management.
 * Set to false to re-enable impostor system.
 */
const DISABLE_IMPOSTORS = true;

export const ROCK_PLANT_ATLAS_CONFIG = {
  /** 16 slots shared between rocks and plants - if exceeded, extras won't render */
  MAX_SLOTS: 16,
  /** Atlas texture size per slot */
  ATLAS_SIZE: 512,
  /** Octahedral grid: 12x6 for rocks/plants (smaller than trees) */
  GRID_SIZE_X: 12,
  GRID_SIZE_Y: 6,
  /** Maximum instances across all presets */
  MAX_INSTANCES: 4000,
} as const;
...
// ============================================================================
// ATLASED ROCK/PLANT IMPOSTOR MANAGER
// ============================================================================

export class AtlasedRockPlantImpostorManager {
  private static instance: AtlasedRockPlantImpostorManager | null = null;
  ...
  private stats = {
    slotsUsed: 0,
    slotsTotal: ROCK_PLANT_ATLAS_CONFIG.MAX_SLOTS,
    rockSlots: 0,
    plantSlots: 0,
    instancesVisible: 0,
    presetsRegistered: 0,
  ...
  private constructor(world: World) {
    ...
    this.slotIndices = new Float32Array(ROCK_PLANT_ATLAS_CONFIG.MAX_INSTANCES);

    // Initialize slots
    const { MAX_SLOTS } = ROCK_PLANT_ATLAS_CONFIG;
  ...
  static getInstance(world: World): AtlasedRockPlantImpostorManager {
    if (!AtlasedRockPlantImpostorManager.instance) {
      AtlasedRockPlantImpostorManager.instance =
        new AtlasedRockPlantImpostorManager(world);
    }
    return AtlasedRockPlantImpostorManager.instance;
  }
  ...
  async init(): Promise<boolean> {
    // Impostors disabled - skip all initialization
    ...
    if (!this.impostorManager.initBaker()) {
      console.warn(
        "[AtlasedRockPlantImpostorManager] Cannot init: baker not ready",
      );
      return false;
    }
    ...
    const { MAX_SLOTS, ATLAS_SIZE } = ROCK_PLANT_ATLAS_CONFIG;
    console.log(
      `[AtlasedRockPlantImpostorManager] Init: ${MAX_SLOTS} slots, ${ATLAS_SIZE}x${ATLAS_SIZE}`,
    );
    return true;
  }
  ...
  private createInstancedMesh(): void {
    const { MAX_INSTANCES, GRID_SIZE_X, GRID_SIZE_Y } = ROCK_PLANT_ATLAS_CONFIG;
    ...
    this.instancedMesh.name = "AtlasedRockPlantImpostors";
  ...
  private createMaterial(
    ...
    material.colorNode = Fn(() => {
      ...
      // The impostor is a billboard facing the camera, so we construct TBN
      ...
      // Build TBN matrix using TSL cross function
      const T = normalize(cross(worldUp, viewDir));
      const B = normalize(cross(viewDir, T));

      // Transform view-space normal to world-space:
      // worldNormal = T * viewNormal.x + B * viewNormal.y + viewDir * viewNormal.z
      const worldNormal = normalize(
        add(
          add(mul(T, viewNormal.x), mul(B, viewNormal.y)),
          mul(viewDir, viewNormal.z),
        ),
      );

      // Lambert diffuse lighting with N dot L
      const L = normalize(this.uLightDir!);
      const NdotL = dot(worldNormal, L);

      // Use clamped NdotL instead of half-Lambert for more pronounced lighting response
      // This makes the rock surfaces more visually responsive to light direction
      const clampedNdotL = max(NdotL, float(0));
      // Blend between ambient floor and full diffuse for softer shadows
      const diffuseFactor = add(mul(clampedNdotL, float(0.7)), float(0.3));
  ...
  /**
   * Check if a slot is available before registering.
   */
  ...
  /**
   * Register a rock or plant preset for atlasing.
   * Returns false if no slot is available (preset won't render as impostor).
   */
  async registerPreset(
    ...
    if (existing) {
      // Already registered - wait for bake if in progress
      if (existing.bakePromise) await existing.bakePromise;
      return existing.slotIndex >= 0;
    }

    // Check slot availability BEFORE starting expensive bake
    if (!this.hasAvailableSlot()) {
      console.warn(
        `[AtlasedRockPlantImpostorManager] All ${ROCK_PLANT_ATLAS_CONFIG.MAX_SLOTS} slots full, ${presetId} (${category}) will not render as impostor`,
      );
      this.stats.presetsSkipped++;
      return false;
    }
    ...
    await preset.bakePromise;

    return preset.slotIndex >= 0;
  }
  ...
  private async bakePreset(
    ...
    try {
      const { ATLAS_SIZE, GRID_SIZE_X, GRID_SIZE_Y } = ROCK_PLANT_ATLAS_CONFIG;
      const result = await this.impostorManager.getOrCreate(
        `atlased_rp_${presetId}_v1`,
        sourceMesh,
        {
      ...
      // Allocate slot now that bake is ready
      this.allocateSlot(presetId);

      console.log(
        `[AtlasedRockPlantImpostorManager] Baked ${preset.category}: ${presetId}`,
      );
    } catch (err) {
      console.error(
        `[AtlasedRockPlantImpostorManager] Bake failed for ${presetId}:`,
        err,
      );
    }
  ...
  private allocateSlot(presetId: string): number {
    ...
    // Find empty slot - NO eviction
    const slotIndex = this.slots.findIndex((s) => s.presetId === null);
    if (slotIndex === -1) {
      console.warn(
        `[AtlasedRockPlantImpostorManager] All ${ROCK_PLANT_ATLAS_CONFIG.MAX_SLOTS} slots full, ${presetId} will not render`,
      );
      this.stats.presetsSkipped++;
      return -1;
    }
    ...
    console.log(
      `[AtlasedRockPlantImpostorManager] Slot ${slotIndex}/${ROCK_PLANT_ATLAS_CONFIG.MAX_SLOTS} <- ${preset.category}:${presetId}`,
    );
    return slotIndex;
  }
  ...
  private updateStats(): void {
    ...
    this.stats.rockSlots = this.slots.filter(
      (s) => s.category === "rock",
    ).length;
    this.stats.plantSlots = this.slots.filter(
      (s) => s.category === "plant",
    ).length;
  }
  ...
  private uploadAtlasToSlot(presetId: string, slotIndex: number): void {
    ...
    if (!preset?.bakeResult) {
      console.warn(
        `[AtlasedRockPlantImpostorManager] No bake result for ${presetId}`,
      );
      return;
    }
    ...
    console.log(
      `[AtlasedRockPlantImpostorManager] Upload ${presetId} to slot ${slotIndex}: ` +
    ...
    if (hasRenderTarget) {
      ...
      this.readAndUploadAtlas(renderer, renderTarget, slotIndex, "albedo")
        ...
        .then(() => {
          ...
          console.log(
            `[AtlasedRockPlantImpostorManager] Uploaded slot ${slotIndex} from render target (${presetId})`,
          );
          this.onSlotLoaded?.(slotIndex, presetId);
        })
        .catch((err) =>
          console.error(
            `[AtlasedRockPlantImpostorManager] Upload failed:`,
            err,
          ),
        );
      ...
      this.uploadTextureToSlot(atlasTexture, slotIndex, "albedo")
        ...
        .then(() => {
          this.slots[slotIndex].loaded = true;
          console.log(
            `[AtlasedRockPlantImpostorManager] Uploaded slot ${slotIndex} from cached texture (${presetId})`,
          );
          this.onSlotLoaded?.(slotIndex, presetId);
        })
        .catch((err) =>
          console.error(
            `[AtlasedRockPlantImpostorManager] Texture upload failed:`,
            err,
          ),
        );
    } else {
      console.warn(
        `[AtlasedRockPlantImpostorManager] No render target or texture for ${presetId}`,
      );
    }
  }
  ...
  private async uploadTextureToSlot(
    ...
    const { ATLAS_SIZE } = ROCK_PLANT_ATLAS_CONFIG;
    const targetArray =
    ...
    if (!ctx) {
      console.warn(`[AtlasedRockPlantImpostorManager] Cannot get 2D context`);
      return;
    }
    ...
    if (
      ...
      console.warn(
        `[AtlasedRockPlantImpostorManager] Unsupported texture image type`,
      );
      return;
    }
  ...
  private async readAndUploadAtlas(
    ...
    if (!asyncRenderer.readRenderTargetPixelsAsync) {
      console.warn(
        `[AtlasedRockPlantImpostorManager] Async read not available`,
      );
      return;
    }
    ...
    if (type === "normal") {
      ...
      console.log(
        `[AtlasedRockPlantImpostorManager] Read ${type} slot ${slotIndex}: ${coverage}% coverage, ${normalVariation}% normal variation`,
      );
    } else {
      console.log(
        `[AtlasedRockPlantImpostorManager] Read ${type} slot ${slotIndex}: ${coverage}% coverage`,
      );
    }
  ...
  /**
   * Add an instance. Returns false if preset has no slot (won't render).
   */
  addInstance(
    presetId: string,
    instanceId: string,
    ...
    const preset = this.presets.get(presetId);
    if (!preset) {
      // Preset not registered - caller should register first
      return false;
    }

    // If preset has no slot, instance won't render
    if (preset.slotIndex < 0) {
      return false;
    }
    ...
    // Show if slot loaded
    if (this.slots[preset.slotIndex].loaded) {
  ...
  private showInstance(instance: ObjectInstance): void {
    ...
    if (idx >= ROCK_PLANT_ATLAS_CONFIG.MAX_INSTANCES) {
      console.warn(`[AtlasedRockPlantImpostorManager] Max instances reached`);
      return;
    }
  ...
  private hideInstance(instance: ObjectInstance): void {
    if (!this.instancedMesh || instance.instanceIndex < 0) return;
  ...
  hasSlot(presetId: string): boolean {
    return (this.presets.get(presetId)?.slotIndex ?? -1) >= 0;
  }

  getSlot(presetId: string): number {
    return this.presets.get(presetId)?.slotIndex ?? -1;
  }

  setOnSlotLoaded(cb: SlotCallback): void {
  ...
  printDiagnostics(): void {
    const { ATLAS_SIZE, MAX_SLOTS } = ROCK_PLANT_ATLAS_CONFIG;
    console.log("=== AtlasedRockPlantImpostorManager Diagnostics ===");
    console.log(
      `Instances: ${this.instanceCount}/${ROCK_PLANT_ATLAS_CONFIG.MAX_INSTANCES}`,
    );
    ...
    if (this.atlasArray) {
      ...
      let nonZeroLayers = 0;
      for (let layer = 0; layer < MAX_SLOTS; layer++) {
  ...
  dispose(): void {
    ...
    this.presets.clear();
    this.slots = [];
    AtlasedRockPlantImpostorManager.instance = null;
    console.log("[AtlasedRockPlantImpostorManager] Disposed");
  }
}

export default AtlasedRockPlantImpostorManager;
