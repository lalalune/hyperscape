...
export class MusicSystem extends SystemBase {
  ...
  private async loadMusicManifest(): Promise<void> {
    ...
    this.logger.info(`Loaded ${this.tracks.length} music tracks (${this.normalTracks.length} normal, ${this.combatTracks.length} combat)`);
  }
  ...
  private async fadeOut(playingTrack: PlayingTrack, duration: number): Promise<void> {
    ...
    try {
      ...
      // Source already stopped or in invalid state
    }
