...
export class TerrainLODManager {
  ...
  update(playerPosition: THREE.Vector3): ChunkUpdate {
    ...
    const { minLod, maxLod, lod1Range, enableGeomorphing, morphRange } =
      this.config;
      ...
      for (let dz = -range; dz <= range; dz++) {
        for (let dx = -range; dx <= range; dx++) {
          ...
          const distance = Math.sqrt(
            (worldX - playerPosition.x) ** 2 + (worldZ - playerPosition.z) ** 2,
          );
          ...
          if (enableGeomorphing && distance > morphStartDist) {
            morphFactor =
              1.0 -
              (distance - morphStartDist) / (lodBoundaryDist - morphStartDist);
  ...
  private calculateScreenError(distance: number, lod: number): number {
    ...
    const screenError =
      (geometricError * screenHeight) / (2 * distance * Math.tan(fovRad / 2));
