...
export async function batchInventoryOperations(
  ...
  const receipt = await mudClient.publicClient.waitForTransactionReceipt({
    hash,
  });
...
async function executeSingleOperation(
  ...
  if (
    ...
    return mudClient.InventorySystem.addItem(
      op.playerAddress,
      op.itemId,
      op.quantity,
    );
  ...
  if (
    ...
    return mudClient.InventorySystem.removeItem(
      op.playerAddress,
      op.slot,
      op.quantity,
    );
  ...
  if (
    ...
    return mudClient.InventorySystem.moveItem(
      op.playerAddress,
      op.fromSlot,
      op.toSlot,
    );
 ...
 */
export function onMudEvent(
  eventType: MudEventType,
  handler: MudEventHandler,
): () => void {
...
export function getIntegrationStatus(): {
  ...
  return {
    ...
    economy:
      !!getOptionalAddress("GOLD_ADDRESS") &&
      !!getOptionalAddress("ITEMS_ADDRESS"),
