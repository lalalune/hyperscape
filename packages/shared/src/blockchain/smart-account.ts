 ...
 */
export const SIMPLE_ACCOUNT_FACTORY = (process.env
    ...
    async estimateUserOperationGas(
      ...
      const response = await fetch(bundlerUrl, {
        ...
        body: JSON.stringify({
          ...
          params: [
            formatUserOpForRpc(userOp as UserOperation),
      ...
      return {
        ...
        verificationGasLimit: BigInt(
          result.result?.verificationGasLimit ?? "0",
        ),
...
export async function createSmartAccountClient(
  ...
  async function buildUserOperation(callData: Hex): Promise<UserOperation> {
 ...
 */
export async function isBundlerAvailable(
  network?: JejuNetwork,
): Promise<boolean> {
  const rpcUrl = getRpcUrl(network);
  const bundlerUrl =
