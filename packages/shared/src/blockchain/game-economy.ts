...
function getGoldAddress(): Address {
  ...
  if (!address) {
    throw new Error(
      "GOLD_ADDRESS not configured. Set the environment variable to enable Gold integration.",
    );
  }
  return address;
}
...
function getItemsAddress(): Address {
  ...
  if (!address) {
    throw new Error(
      "ITEMS_ADDRESS not configured. Set the environment variable to enable Items integration.",
    );
  }
  return address;
}
 ...
 */
export async function getGoldClaimNonce(
  playerAddress: Address,
): Promise<bigint> {
  const client = getClient();
  const goldAddress = getGoldAddress();
...
export async function watchItemBurns(
  ...
  const unwatch = client.watchContractEvent({
    ...
    onLogs: async (
      logs: Array<{
        args: { player: Address; itemId: bigint; amount: bigint };
