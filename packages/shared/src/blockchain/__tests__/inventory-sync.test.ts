...
describe("MUD Inventory Sync", () => {
  ...
  describe("Inventory Add Item Flow", () => {
    ...
    test("addItem validates itemId exists in ItemMetadata table", async () => {
      ...
      const mockAddItemInvalid = mock(() => {}).mockRejectedValue(
    ...
    test("addItem handles stackable items correctly", async () => {
      ...
      const mockAddStackable = mock(() => {}).mockImplementation(
        (_player, itemId, quantity) => {
  ...
  describe("Inventory Remove Item Flow", () => {
    ...
    test("removeItem clears slot when quantity reaches 0", async () => {
      ...
      const mockRemoveAll = mock(() => {}).mockImplementation(
        (_player, _slot, quantity) => {
...
describe("NFT Minting (Items.sol)", () => {
  describe("Mint Flow", () => {
    test("server signs mint request with item metadata", async () => {
      const { signItemMint, generateInstanceId } =
    ...
    test("non-stackable items require unique instanceId", async () => {
      ...
      const mockMintDuplicate = mock(() => {}).mockImplementation(
        ({ instanceId: id }) => {
      ...
      expect(() => mockMintDuplicate({ instanceId })).toThrow(
        "InstanceAlreadyMinted",
      );
    });
    ...
    test("tracks original minter for provenance", async () => {
      ...
      const mintedMetadata = new Map<
        string,
        { originalMinter: Address; mintedAt: number }
      >();

      const mockMintWithProvenance = mock(() => {}).mockImplementation(
        ({ minter, itemId }) => {
    ...
    test("burning NFT converts back to in-game item", async () => {
      ...
      const mockBurn = mock(() => {}).mockResolvedValue({
        ...
        events: [
          {
            name: "ItemBurned",
            args: { player: TEST_ADDRESS, itemId: 1n, amount: 1n },
          },
        ],
      });
...
describe("Gold Bridge (Gold.sol)", () => {
  describe("Claim Flow", () => {
    ...
    test("nonce prevents replay attacks", async () => {
      ...
      const mockClaim = mock(() => {}).mockImplementation(
        ({ player, nonce }) => {
      ...
      // Replay fails
      expect(() => mockClaim({ player: TEST_ADDRESS, nonce: 0 })).toThrow(
        "InvalidNonce",
      );
    });
  });
...
describe("Loot Drop & Ephemeral Items", () => {
  ...
  describe("Death Drop Flow", () => {
    test("death clears inventory and creates headstone", () => {
      ...
      function dropAllItems(inv: PlayerInventory): {
        droppedItems: typeof inv.items;
      } {
...
describe("Security Vulnerabilities", () => {
  describe("Item Duplication Exploits", () => {
    ...
    test("VULN-005: NFT double-mint - instanceId tracking", () => {
      ...
      // Second mint fails
      expect(() => mintItem("unique_001", false)).toThrow(
        "InstanceAlreadyMinted",
      );
    });
  ...
  describe("Scam Prevention", () => {
    test("VULN-009: Fake item display - on-chain verification", () => {
      ...
      function verifyItemBeforeTrade(
        itemId: number,
        claimedRarity: number,
      ): boolean {
