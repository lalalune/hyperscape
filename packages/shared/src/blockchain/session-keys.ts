...
export const GAME_PERMISSION_SETS = {
  ...
  INVENTORY: (worldAddress: Address): SessionKeyPermission[] => [
    {
      target: worldAddress,
      selector: selector("hyperscape__addItem(address,uint16,uint32)"),
    },
    {
      target: worldAddress,
      selector: selector("hyperscape__removeItem(address,uint8,uint32)"),
    },
    {
      target: worldAddress,
      selector: selector("hyperscape__moveItem(address,uint8,uint8)"),
    },
  ],
  ...
  EQUIPMENT: (worldAddress: Address): SessionKeyPermission[] => [
    {
      target: worldAddress,
      selector: selector("hyperscape__equipItem(uint8)"),
    },
    {
      target: worldAddress,
      selector: selector("hyperscape__unequipItem(uint8)"),
    },
  ],
  ...
  COMBAT: (worldAddress: Address): SessionKeyPermission[] => [
    {
      target: worldAddress,
      selector: selector("hyperscape__attackMob(address)"),
    },
  ],
  ...
  GATHERING: (worldAddress: Address): SessionKeyPermission[] => [
    {
      target: worldAddress,
      selector: selector("hyperscape__chopTree(address)"),
    },
...
export function hasPermission(
  ...
  for (const permission of sessionKey.permissions) {
    const targetMatch =
      permission.target.toLowerCase() === target.toLowerCase();
    const selectorMatch =
      permission.selector === "*" ||
    ...
    if (targetMatch && selectorMatch) {
      ...
      if (
        permission.maxCalls &&
        sessionKey.transactionCount >= permission.maxCalls
      ) {
        continue;
      }
      return true;
...
export async function executeWithSessionKey(
  ...
  if (!hasPermission(sessionKey, target, funcSelector)) {
    throw new Error(
      `Session key does not have permission for ${target}:${funcSelector}`,
    );
  }
...
export class SessionKeyManager {
  ...
  constructor(
    store: SessionKeyStore = defaultSessionKeyStore,
    network?: JejuNetwork,
  ) {
    this.store = store;
  ...
  async createSession(
    ...
    const { privateKey, address } =
      existingSessionKey || createSessionKeyPair();
  ...
  async execute(
    ...
    const hash = await executeWithSessionKey(
      session,
      target,
      callData,
      this.network,
