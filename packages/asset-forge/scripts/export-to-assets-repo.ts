...
// Configuration
const ASSETS_REPO_PATH = process.env.ASSETS_REPO_PATH || '/Users/home/Downloads/assets-main'
...
async function exportApprovedAssets() {
  ...
  try {
    ...
    console.error('Set ASSETS_REPO_PATH environment variable to your local assets repo clone')
    process.exit(1)
  }
  ...
  for (const asset of approvedAssets) {
    try {
      ...
      console.log(`  ✓ Copied: ${glbFile} → models/${asset.type}/${asset.name}.glb`)
      ...
      await db
        ...
        .set({
          ...
          manifestPath: `manifests/${asset.type}s.json`,
          updatedAt: new Date(),
        })
...
async function updateManifest(assetType: string, entry: ManifestEntry) {
  const manifestPath = path.join(ASSETS_REPO_PATH, 'manifests', `${assetType}s.json`)
  ...
  const idField = assetType === 'character' ? 'characterId' : assetType === 'item' ? 'itemId' : 'id'
  const existingIndex = entries.findIndex((e: any) => e[idField] === entry[idField])
