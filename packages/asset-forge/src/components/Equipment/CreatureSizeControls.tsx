        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-text-secondary mt-0.5">
              Adjust avatar size and weapon scaling
            </p>
          </div>
        </div>
      </div>
          ...
          {!autoScaleWeapon && (
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-mono text-primary">
                  {weaponScaleOverride.toFixed(1)}x
                </span>
              </div>
              <RangeInput
                type="range"
                ...
                onChange={(e) =>
                  setWeaponScaleOverride(parseFloat(e.target.value))
                }
              />
            </div>
          )}
          ...
          {autoScaleWeapon && selectedEquipment && (
            <div className="p-5 bg-bg-tertiary/20 rounded-lg border border-border-primary space-y-2">
              <p className="text-xs text-text-tertiary flex items-center gap-1">
                ...
                {
                  CREATURE_SIZE_CATEGORIES[
                    creatureCategory as keyof typeof CREATURE_SIZE_CATEGORIES
                  ].name
                }
                :
              </p>
              <div className="flex flex-wrap gap-1">
                {CreatureScalingService.getRecommendedWeapons(avatarHeight).map(
                  (weapon) => (
