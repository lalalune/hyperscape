...
export const AvatarArmorDemo: React.FC<AvatarArmorDemoProps> = ({
  ...
  const _mixer = useRef<THREE.AnimationMixer | undefined>(undefined);
  const _lastTime = useRef(0);
  const _activeAction = useRef<THREE.AnimationAction | null>(null);
  const _animationFrame = useRef<number | undefined>(undefined);

  // Check if paths are valid before attempting to load
  const hasValidPaths =
    avatarPath && armorPath && avatarPath !== "" && armorPath !== "";
  ...
  const animationPath = useMemo(() => {
    if (needsAnimationFile && avatarPath) {
      ...
      if (match) {
        ...
        console.log(`[AvatarArmorDemo] Looking for animation file: ${path}`);
        return path;
      } else {
        console.log(
          `[AvatarArmorDemo] Path doesn't match gdd-assets pattern, skipping animation loading for: ${avatarPath}`,
        );
      }
    }
  ...
  try {
    ...
    console.warn(
      `[AvatarArmorDemo] Animation file not found: ${animationPath}`,
      error,
    );
    animationGltf = null;
  }
    ...
    return (
      <group>
        <Html center>
          ...
          >
            Please select both an avatar and armor from the Asset Selection
            panel
          </div>
        </Html>
      </group>
    ...
    if (avatarClone && armorClone && !isLoaded) {
      ...
      if (avatarMesh && armorMesh) {
        ...
        if (avatarMinY !== 0) {
          ...
          console.log(
            `Adjusted avatar Y position by ${-avatarMinY} to stand on grid`,
          );
        }
        ...
        console.log(
          "Avatar setup bounds:",
          setupAvatarBounds.getSize(new THREE.Vector3()),
        );
        console.log(
          "Armor setup bounds:",
          setupArmorBounds.getSize(new THREE.Vector3()),
        );
        console.log("========================");
        ...
        // Verify final bounds
        const finalAvatarBounds = new THREE.Box3().setFromObject(
          finalAvatarMesh,
        );
      ...
      for (const clip of animations) {
        ...
        if (currentAnimation === "walking") {
          if (
            ...
            !clipName.includes("run") &&
            !clipName.includes("running")
          ) {
            targetClip = clip;
        ...
        // For running, prefer "run" but exclude "walk"
          ...
          if (
            ...
            !clipName.includes("walk") &&
            !clipName.includes("walking")
          ) {
            targetClip = clip;
