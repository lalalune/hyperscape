  ...
  useEffect(() => {
    const checkHealth = async () => {
      ...
      if (data) {
        ...
        setStatus(`${data.status} - Meshy: ${data.services.meshy}, OpenAI: ${data.services.openai}`)
      }
    }
...
export function RetextureExample({ baseAssetId }: { baseAssetId: string }) {
  ...
  const handleRetexture = async () => {
    ...
    if (error) {
      ...
      alert(`Retexture task created: ${data.taskId}`)
    }
...
export function SaveSpritesExample({ assetId }: { assetId: string }) {
  const [sprites, setSprites] = useState<Array<{ angle: number; imageData: string }>>([])
  ...
  return (
    <div>
      ...
      <button onClick={handleSaveSprites} disabled={saving || sprites.length === 0}>
