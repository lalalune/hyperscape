...
export class ArmorFittingService {
  ...
  computeBodyRegions(
    ...
    for (const [regionName, patterns] of Object.entries(regionPatterns)) {
      ...
      skeleton.bones.forEach((bone, index) => {
        ...
        if (
          patterns.some((pattern) => boneName.includes(pattern.toLowerCase()))
        ) {
      ...
      if (regionVertexPositions.length > 10) {
        ...
        const influence =
          influenceRadius[regionName as keyof typeof influenceRadius] || 0.2;
        ...
        regionBones.forEach((bone) => {
          ...
          boundingBox.expandByPoint(
            boneWorldPos
              .clone()
              .add(new Vector3(influence, influence, influence)),
          );
          boundingBox.expandByPoint(
            boneWorldPos
              .clone()
        ...
        if (regionName === "torso" && size.y < avatarSize.y * 0.2) {
          ...
          if (spinePositions.length > 0) {
            ...
            spinePositions.forEach((pos) => {
              boundingBox.expandByPoint(
                pos.clone().add(new Vector3(0.4, 0.3, 0.4)),
              );
              boundingBox.expandByPoint(
                pos.clone().add(new Vector3(-0.4, -0.3, -0.4)),
              );
  ...
  resolveCollisions(
    ...
    for (let iter = 0; iter < iterations; iter++) {
      ...
      collisions.forEach((collision) => {
        ...
        const displacement = collision.normal
          .clone()
        ...
        for (let j = 0; j < position.count; j++) {
          ...
          if (distance < influenceRadius) {
            ...
            const smoothedDisplacement = displacement
              .clone()
              .multiplyScalar(weight * 0.5);
    ...
    for (let pass = 0; pass < passes; pass++) {
      ...
      for (let i = 0; i < position.count; i++) {
        ...
        if (neighbors.length > 0) {
          ...
          smoothedPositions[idx] = THREE.MathUtils.lerp(
            positions[idx],
            avgPos.x,
            strength,
          );
          smoothedPositions[idx + 1] = THREE.MathUtils.lerp(
            positions[idx + 1],
            avgPos.y,
            strength,
          );
          smoothedPositions[idx + 2] = THREE.MathUtils.lerp(
            positions[idx + 2],
            avgPos.z,
            strength,
          );
   ...
   */
  createBodyMesh(positions: Float32Array, indices: Uint32Array | null): Mesh {
      ...
      for (let i = 0; i < armorVertexCount; i++) {
        ...
        for (let j = 0; j < avatarPositions.count; j++) {
          ...
          if (
            ...
            distance < effectiveSearchRadius * 2
          ) {
        ...
        if (closestAvatarIndex !== -1) {
          ...
          for (let j = 0; j < 4; j++) {
            skinIndices[i * 4 + j] =
              avatarSkinIndices.array[closestAvatarIndex * 4 + j];
            skinWeights[i * 4 + j] =
              avatarSkinWeights.array[closestAvatarIndex * 4 + j];
          }
          additionalMapped++;
          nearestMapped++;
        }
  ...
  private static projectiveWeightTransfer(
    ...
    const { maxProjectionDistance = 1.0, fallbackToBoneDistance = true } =
      options;
    ...
    if (intersections.length > 0) {
      ...
      if (face) {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          const baryWeight =
            i === 0 ? baryCoord.x : i === 1 ? baryCoord.y : baryCoord.z;

          // Get weights from this vertex
          for (let j = 0; j < 4; j++) {
            const boneIndex = avatarSkinIndices.array[vertexIndex * 4 + j];
            const weight =
              avatarSkinWeights.array[vertexIndex * 4 + j] * baryWeight;

            if (weight > 0) {
              weightMap.set(
                boneIndex,
                (weightMap.get(boneIndex) || 0) + weight,
              );
            }
          }
        }
  ...
  async exportFittedArmor(
    ...
    const hasScaledBones = skinnedArmorMesh.skeleton.bones.some(
      (bone) =>
    ...
    if (method === "full") {
      ...
      const sortedIndices = Array.from(requiredBoneIndices).sort(
        (a, b) => a - b,
      );
  ...
  /**
   * Export skinned armor for use in game environments (classic fantasy MMORPG-style)
  ...
  async exportSkinnedArmorForGame(
    ...
    sortedIndices.forEach((oldIndex) => {
      const oldBoneWorldMatrix = oldBoneWorldMatrices.get(
        skeleton.bones[oldIndex],
      )!;
  ...
  private getUsedBones(geometry: THREE.BufferGeometry): Set<number> {
    ...
    for (let i = 0; i < skinIndices.count; i++) {
      for (let j = 0; j < 4; j++) {
        ...
        if (weight > 0.001) {
          // Only count bones with meaningful weights
  ...
  /**
   * Equip game-ready armor to a character (classic fantasy MMORPG-style)
  ...
  equipArmorToCharacter(
    ...
    armor.skeleton.bones.forEach((armorBone) => {
      ...
      let matchedBone = characterSkeleton.bones.find(
        (b) => b.name === armorBone.name,
      );

      // Try mapped name
      if (!matchedBone && boneNameMapping[armorBone.name]) {
        const mappedName = boneNameMapping[armorBone.name];
        matchedBone = characterSkeleton.bones.find(
          (b) => b.name === mappedName,
        );
      }
    ...
    for (let i = 0; i < skinIndices.count; i++) {
      for (let j = 0; j < 4; j++) {
        ...
        newSkinIndices[i * 4 + j] =
          charBoneIndex !== undefined ? charBoneIndex : 0;
      }
    }
      ...
      const targetBoneIndex = targetSkeleton.bones.findIndex((bone) => {
        ...
        if (autoDetectMapping) {
          ...
          return variations.some(
            (v) => bone.name.toLowerCase() === v.toLowerCase(),
          );
        }
