      ...
      try {
        ...
        if (
          ...
          try {
            ...
            console.error('❌ Failed to upload image:', (uploadError as Error).message)
        ...
        if (pipeline.config.type === 'character') {
          ...
          try {
            ...
            console.warn('⚠️ Normalization failed, using raw model:', (error as Error).message)
          ...
          try {
            ...
            console.warn('⚠️ Weapon normalization failed, using raw model:', (error as Error).message)
            await fs.copyFile(rawModelPath, normalizedModelPath)
          }
        } else {
        ...
        try {
          ...
          if (riggingResult.result && riggingResult.result.basic_animations) {
            ...
            if (animations.walking_glb_url) {
              ...
              console.error('⚠️ Failed to extract T-pose:', (tposeError as Error).message)
...
Your task is to enhance the user's description to create better results with image generation and 3D conversion.`
   ...
   */
  private async extractTPoseFromAnimation(inputPath: string, outputPath: string): Promise<void> {
