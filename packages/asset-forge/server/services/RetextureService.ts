...
class MeshyClient {
  ...
  async remesh(modelPath: string, options: { targetPolycount?: number } = {}): Promise<{ modelUrl: string; taskId: string }> {
  ...
  async waitForCompletion(taskId: string, progressCallback?: (progress: number) => void): Promise<TaskStatus> {
...
export class RetextureService {
  ...
  constructor(config?: { meshyApiKey?: string; imageServerBaseUrl?: string }) {
  ...
  async retexture({ baseAssetId, materialPreset, outputName, assetsDir, user = null }: RetextureParams) {
    if (!this.meshyClient) {
  ...
  async saveRetexturedAsset({
    ...
    const variantMetadata: AssetMetadataType = {
      ...
      walletAddress: user?.walletAddress || baseMetadata.walletAddress || undefined,
      isPublic: baseMetadata.isPublic !== undefined ? baseMetadata.isPublic : true
    }
  ...
  async updateBaseAssetVariants(baseAssetId: string, variantId: string, assetsDir: string): Promise<void> {
    try {
  ...
  async getAssetMetadata(assetId: string, assetsDir: string): Promise<AssetMetadataType> {
    const metadataPath = path.join(assetsDir, assetId, 'metadata.json')
    return JSON.parse(await fs.readFile(metadataPath, 'utf-8')) as AssetMetadataType
  }
