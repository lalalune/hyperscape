...
export class AssetService {
    ...
    try {
      ...
      for (const assetDir of assetDirs) {
        ...
        try {
          ...
          const metadata = JSON.parse(await fs.readFile(metadataPath, 'utf-8')) as AssetMetadataType
  ...
  async updateDependenciesAfterDelete(deletedAssetId: string): Promise<void> {
    ...
    try {
      const dependencies = JSON.parse(await fs.readFile(dependenciesPath, 'utf-8')) as Dependencies
  ...
  async updateAsset(assetId: string, updates: AssetUpdate, userContext: UserContextType | null = null): Promise<Asset | null> {
    try {
      ...
      const currentMetadata = JSON.parse(await fs.readFile(metadataPath, 'utf-8')) as AssetMetadataType
  ...
  async updateDependenciesAfterRename(oldId: string, newId: string): Promise<void> {
    const dependenciesPath = path.join(this.assetsDir, 'dependencies.json')
