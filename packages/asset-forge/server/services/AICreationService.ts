...
class ImageGenerationService {
  ...
  async generateImage(
    ...
    if (!useAIGateway && !useDirectOpenAI) {
      throw new Error('AI_GATEWAY_API_KEY or OPENAI_API_KEY required for image generation')
    }

    // Load generation prompts
    const generationPrompts: GenerationPrompts | null = await getGenerationPrompts()
    const promptTemplate: string = generationPrompts?.imageGeneration?.base ||
    ...
    const modelName = useAIGateway ? 'google/gemini-2.5-flash-image' : this.model

    console.log(`🎨 Using ${useAIGateway ? 'Vercel AI Gateway' : 'direct OpenAI API'} for image generation (model: ${modelName})`)
...
class MeshyService {
  ...
  async startImageTo3D(imageUrl: string, options: ImageTo3DOptions): Promise<string | any> {
