/**
 * Standardized Error Messages and Codes (Server)
 *
 * Server-side error handling utilities with consistent
 ...
 */
export function createStandardError(code, message, details, requestId) {
  return {
    code,
    message,
    details,
 ...
 */
export function sendErrorResponse(res, code, message, details, requestId) {
  const error = createStandardError(code, message, details, requestId)
  const status = getHttpStatusFromErrorCode(code)
  return res.status(status).json(formatErrorResponse(error))
}
