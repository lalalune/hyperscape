...
import { canModifyAsset } from '../utils/privy'

export const createAssetRoutes = (rootDir: string, assetService: AssetService) => {
  return new Elysia({ prefix: '/api/assets', name: 'assets' })
    .guard({
      ...
      .get('', async () => {
        ...
        detail: {
          ...
          description: 'Returns a list of all generated 3D assets. (Auth optional - shows public assets, authenticated users see their own assets)'
        }
      })
      ...
      .delete('/:id', async ({ params: { id }, query, user, set }) => {
        // Get asset metadata to check ownership
        const assets = await assetService.listAssets()
        const asset = assets.find((a: Models.AssetMetadataType) => a.id === id)
        ...
        if (!canModifyAsset(user, asset)) {
          set.status = 403
          return { error: 'Permission denied. You can only delete your own assets. Admins can delete any asset.' }
        }
        ...
        detail: {
          ...
          description: 'Deletes an asset and optionally its variants. (Auth required - users can only delete their own assets, admins can delete any asset)'
        }
      })
      ...
      .patch('/:id', async ({ params: { id }, body, user, set }) => {
        ...
        detail: {
          ...
          description: 'Updates asset metadata like name, type, tier, etc. (Auth required - users can only update their own assets, admins can update any asset)'
        }
      })

      // Save sprites for an asset
      .post('/:id/sprites', async ({ params: { id }, body }) => {
        const { sprites, config } = body

        console.log(`[Sprites] Saving ${sprites.length} sprites for asset: ${id}`)
        ...
        await Bun.write(metadataPath, JSON.stringify(spriteMetadata, null, 2))
        ...
        await Bun.write(assetMetadataPath, JSON.stringify(updatedMetadata, null, 2))
        ...
        detail: {
          ...
          description: 'Saves generated sprite images and metadata for an asset. (Auth optional)'
        }
      })

      // Upload VRM file
      .post('/upload-vrm', async ({ body }) => {
        ...
        console.log(`[VRM Upload] File size: ${(file.size / 1024 / 1024).toFixed(2)} MB`)
        ...
        detail: {
          ...
          description: 'Uploads a VRM file for an asset. (Auth optional - authenticated users get ownership tracking)'
        }
      })
    )
}
