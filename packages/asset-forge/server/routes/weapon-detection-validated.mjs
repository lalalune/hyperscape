/**
 * Validated Weapon Detection Routes
 * Handles weapon handle and orientation detection with image validation
 */

import { validateBase64Image, validateImageBuffer, FileValidationError } from '../utils/fileValidation.mjs'
 ...
 */
export async function POST_detectWeaponHandle(req, res) {
  try {
    if (!process.env.OPENAI_API_KEY) {
      throw new Error('OpenAI API key not configured')
    }

    const { image, angle, promptHint } = req.body
    ...
    try {
      const buffer = validateBase64Image(image)
      await validateImageBuffer(buffer, { maxSize: 10 * 1024 * 1024 }) // 10MB max for analysis
      console.log(`[WeaponDetect] ✅ Image validated (${formatBytes(buffer.length)})`)
    } catch (error) {
      ...
      `\n\nRespond with ONLY a JSON object in this exact format:
{
  ...
  "detectedParts": {
    ...
    // Use GPT-4 Vision to analyze the weapon
    ...
    console.error('[WeaponDetect] Error:', error)
 ...
 * Detect if weapon is upside down
 */
export async function POST_detectWeaponOrientation(req, res) {
  try {
    if (!process.env.OPENAI_API_KEY) {
      throw new Error('OpenAI API key not configured')
    }
    ...
    try {
      const buffer = validateBase64Image(image)
      await validateImageBuffer(buffer, { maxSize: 10 * 1024 * 1024 }) // 10MB max
      console.log(`[WeaponOrient] ✅ Image validated (${formatBytes(buffer.length)})`)
    } catch (error) {
      if (error instanceof FileValidationError) {
        return res.status(400).json({
          success: false,
...
Respond with ONLY a JSON object:
{
  ...
  "reason": "<brief explanation of your decision>"
    ...
    console.error('[WeaponOrient] Error:', error)
    res.status(500).json({ success: false, error: error.message })
  }
}
