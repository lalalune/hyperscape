...
import { DEG2RAD } from '@hyperscape/shared'
...
self.onmessage = (msg: WorkerMessage) => {
  ...
  switch (data.op) {
    case 'create': {
      // console.log('create!!')
              // Extract only the EmitterConfig properties from data
      const emitterConfig: EmitterConfig = {
        id: data.id || 'default',
        max: data.max as number,
        duration: data.duration as number,
        rate: data.rate as number,
        emitting: data.emitting as boolean,
        bursts: (data.bursts as { time: number; count: number; }[]) || [],
        direction: (data.direction as number) || 0,
        space: (data.space as "world" | "local") || 'world',
        timescale: (data.timescale as number) || 1,
        rateOverDistance: data.rateOverDistance as number | undefined,
        life: (data.life as string) || '1',
        speed: (data.speed as string) || '10',
        size: (data.size as string) || '1',
        rotate: (data.rotate as string) || '0',
        color: (data.color as string) || '#ffffff',
        alpha: (data.alpha as string) || '1',
        emissive: (data.emissive as string) || '0',
        shape: (data.shape as unknown[]) || ['point'],
        spritesheet: data.spritesheet as [number, number, number, boolean] | undefined,
        force: data.force as number[] | undefined,
        velocityLinear: data.velocityLinear as number[] | undefined,
        velocityOrbital: data.velocityOrbital as number[] | undefined,
        velocityRadial: data.velocityRadial as number | undefined,
        sizeOverLife: data.sizeOverLife as string | undefined,
        rotateOverLife: data.rotateOverLife as string | undefined,
        colorOverLife: data.colorOverLife as string | undefined,
        alphaOverLife: data.alphaOverLife as string | undefined,
        emissiveOverLife: data.emissiveOverLife as string | undefined,
        blending: data.blending as string | undefined,
        loop: (data.loop as boolean) !== false
      };
      const system = createEmitter(emitterConfig)
    ...
    //   break
    case 'update': {
      // Create UpdateData with required properties
      const updateData: UpdateData = {
        delta: data.delta as number,
        camPosition: data.camPosition as number[],
        matrixWorld: data.matrixWorld as number[],
        aPosition: data.aPosition as Float32Array,
        aRotation: data.aRotation as Float32Array,
        aDirection: data.aDirection as Float32Array,
        aSize: data.aSize as Float32Array,
        aColor: data.aColor as Float32Array,
        aAlpha: data.aAlpha as Float32Array,
        aEmissive: data.aEmissive as Float32Array,
        aUV: data.aUV as Float32Array
      };
      emitters[data.emitterId!]?.update(updateData)
      break
    }
    // case 'emitCustom':
...
function createEmitter(config: EmitterConfig): ParticleEmitter {
  ...
  function update({
    ...
    m1.fromArray(matrixWorld)
    v1.fromArray(camPosition)
    elapsed += delta
    // track move direction and distance
    const currWorldPos = v2.setFromMatrixPosition(m1)
    let distanceMoved
    if (lastWorldPos) {
    ...
    if (emitting) {
      ...
      if (amount > 0) emit({ amount, matrixWorld: m1 })
    ...
    while (bursts.length && bursts[0].time <= elapsed) {
      ...
      if (burst) {
        emit({ amount: burst.count, matrixWorld: m1 })
      }
    }
    // update particles
    for (const particle of particles) {
      ...
      if (velocityLinear) {
        ...
        if (config.space === 'world') {
          ...
          v3.applyQuaternion(q1.setFromRotationMatrix(m1))
          particle.position.add(v3)
        }
      }
      // orbital velocity
      ...
      if (config.space === 'local') {
        particle.finalPosition.applyMatrix4(m1)
      }
      // particle.distance = particle.worldPosition.distanceToSquared(camPosition)
      particle.distance = particle.position.distanceToSquared(v1)
...
function createSpritesheet(options: [number, number, number, boolean] | null | undefined) {
