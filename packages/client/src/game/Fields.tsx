...
export function FieldText({
  ...
  return (
    ...
    >
      <div className="field-label text-[8px] mb-1 text-white/70 font-medium">
        {label}
      </div>
      <input
        type="text"
...
export function FieldTextarea({
  ...
  return (
    <label
      ...
      onPointerLeave={() => hint && setHint(null)}
    >
      <div className="field-label text-[0.8125rem] mb-1.5 text-white/70 font-medium">
        {label}
      </div>
      <textarea
        ref={textareaRef}
  ...
  return (
    ...
    >
      ...
      <div className="field-switch-control flex items-center gap-1.5 text-[8px]">
        ...
        <div className="field-switch-value flex-1 text-center text-white">
          {options[idx]?.label || ""}
        </div>
        <div
          className="field-switch-btn w-5 h-5 flex items-center justify-center bg-white/5 border border-white/10 rounded cursor-pointer hover:bg-white/10 hover:border-white/20"
...
export function FieldRange({
  ...
  useEffect(() => {
    ...
    if (!track) return;
    function calculateValueFromPointer(
      e: PointerEvent,
    ...
    function onPointerUp(e: PointerEvent) {
      ...
      sliding = false;
      setSliding(false);
      const finalValue = calculateValueFromPointer(
        e,
        e.currentTarget as HTMLElement,
      );
      setLocal(finalValue);
  ...
  return (
    ...
    >
      <div className="fieldrange-label flex-1 whitespace-nowrap overflow-hidden text-ellipsis text-[8px] text-white/70 pr-2">
        {label}
      </div>
      <div
        className={`fieldrange-text text-[7px] font-medium text-white mr-1.5 ${isHovered ? "opacity-100" : "opacity-0"}`}
      >
        {text}
      </div>
      <div
        className="fieldrange-track w-20 shrink-0 h-1.5 rounded-sm flex items-stretch bg-white/10 cursor-pointer"
        ref={trackRef}
      >
        <div
...
export function FieldFile({
  ...
  return (
    ...
    >
      <div className="fieldfile-label flex-1 whitespace-nowrap overflow-hidden text-ellipsis pr-4 text-[0.9375rem] text-white/70">
        {label}
      </div>
      {!value && !loading && (
        <div className="fieldfile-placeholder text-white/50">
          {kind.placeholder}
        </div>
      )}
      {name && (
        <div className="fieldfile-name text-[0.9375rem] text-right whitespace-nowrap overflow-hidden text-ellipsis max-w-36 text-white">
          {name}
        </div>
      )}
      {!!value && !loading && (
...
export function FieldNumber({
  ...
  return (
    ...
    >
      <div className="fieldnumber-label w-[9.4rem] shrink-0 whitespace-nowrap overflow-hidden text-ellipsis text-[0.9375rem] text-white/70">
        {label}
      </div>
...
export function FieldVec3({
  ...
  return (
    ...
    >
      <div className="fieldvec3-label w-[9.4rem] shrink-0 whitespace-nowrap overflow-hidden text-ellipsis text-[0.9375rem] text-white/70">
        {label}
      </div>
...
export function FieldCurve({
  ...
  const setHint = hintContext.setHint;
  const curve = useMemo(
    () => new Curve().deserialize(value || "0,0.5,0,0|1,0.5,0,0"),
    [value],
  );
  ...
  return (
    ...
    >
      ...
      >
        <div className="fieldcurve-label flex-1 whitespace-nowrap overflow-hidden text-ellipsis pr-4 text-[0.9375rem] text-white/70">
          {label}
        </div>
...
export function FieldBtn({ label, note, hint, nav, onClick }: FieldBtnProps) {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <div className="fieldbtn-label flex-1 whitespace-nowrap overflow-hidden text-ellipsis text-[8px] text-white/70">
        {label}
      </div>
      {note && (
        <div className="fieldbtn-note text-[8px] text-white/50">{note}</div>
      )}
