  ...
  useEffect(() => {
    const handleGatheringStart = (data: unknown) => {
      const d = data as {
        ...
        action?: string;
        duration?: number;
        cycleTicks?: number; // tile-based-MMORPG-style tick count
      ...
      // tile-based MMORPG standard: 4 ticks per attempt = 2.4 seconds
      const tickDuration = d.tickDurationMs || 600;
      const cycleTicks = d.cycleTicks || 4; // Default to tile-based MMORPG standard 4 ticks
