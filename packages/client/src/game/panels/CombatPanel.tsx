...
// This enables instant display when reopening panel (classic fantasy MMORPG pattern)
...
export function CombatPanel({ world, stats, equipment }: CombatPanelProps) {
  ...
  // Auto-retaliate state (classic MMORPG default is ON)
  const [autoRetaliate, setAutoRetaliate] = useState<boolean>(() => {
    ...
    return true; // classic MMORPG default: ON
  });
  const playerId = world.entities?.player?.id ?? null;
  const previousPlayerIdRef = useRef<string | null>(null);

  // Calculate combat level using classic combat formula (melee-only MVP)
  ...
  const changeStyle = (next: string) => {
    ...
    // Optimistic: update UI instantly (classic MMORPG has zero visible delay)
    ...
    // rules-accurate: selecting autocast opens the spells panel for spell selection
    if (next === "autocast") {
  ...
  // Filter styles based on equipped weapon (rules-accurate restrictions)
