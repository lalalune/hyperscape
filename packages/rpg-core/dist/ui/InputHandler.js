...
exports.InputHandler = exports.InputEventType = void 0;
var InputEventType;
(function (InputEventType) {
    InputEventType["MOUSE_MOVE"] = "mouse_move";
    InputEventType["MOUSE_DOWN"] = "mouse_down";
    InputEventType["MOUSE_UP"] = "mouse_up";
    InputEventType["MOUSE_WHEEL"] = "mouse_wheel";
    InputEventType["KEY_DOWN"] = "key_down";
    InputEventType["KEY_UP"] = "key_up";
    InputEventType["TOUCH_START"] = "touch_start";
    InputEventType["TOUCH_MOVE"] = "touch_move";
    InputEventType["TOUCH_END"] = "touch_end";
...
class InputHandler {
    ...
    handleMouseMove(event) {
        ...
        if (this.dragStart && !this.isDragging) {
            const distance = Math.sqrt(Math.pow(x - this.dragStart.x, 2) +
    ...
    handleKeyDown(event) {
        ...
        if (this.state.keyboard.keys.has(event.key))
            return;
    ...
    handleTouchEnd(event) {
        ...
        for (const touch of event.changedTouches) {
            ...
            if (index !== -1) {
                ...
                const distance = Math.sqrt(Math.pow(touchPoint.position.x - touchPoint.startPosition.x, 2) +
