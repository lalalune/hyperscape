...
exports.CombatAnimationSystem = void 0;
const sdk_1 = require("@hyperfy/sdk");
// THREE import removed
const CombatDefinitions_1 = require("./CombatDefinitions");
class CombatAnimationSystem extends sdk_1.System {
    constructor(world) {
        super(world);
    ...
    playAnimation(entityId, animationId, onComplete) {
        const entity = this.world.getEntityById(entityId);
        if (!entity) {
            return '';
        }
        const animationDef = CombatDefinitions_1.COMBAT_ANIMATIONS[animationId];
        if (!animationDef) {
            return '';
        }
    ...
    createProjectile(startPos, endPos, projectileType) {
        ...
        if (visualSystem && visualSystem.addMesh) {
            ;
            visualSystem.addMesh(projectile);
        }
        const effect = {
            id: effectId,
            type: 'ranged_projectile',
    ...
    createMagicParticles(position, spellType) {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            particle.position.set(position.x + (Math.random() - 0.5) * 2, position.y + 1 + Math.random(), position.z + (Math.random() - 0.5) * 2);
            // Random velocity
    ...
    createSpellProjectile(startPos, endPos, spellType) {
    ...
    createHitSplat(position, damage, damageType) {
    ...
    getEntityMesh(entityId) {
        ...
        return visualSystem.getEntityMesh?.(entityId) || null;
    }
    isAnimationPlaying(entityId) {
    ...
    updateAnimationFrame(animation, elapsed) {
        if (!animation.mesh) {
            return;
        }
        const animationDef = CombatDefinitions_1.COMBAT_ANIMATIONS[animation.animationId];
        if (!animationDef) {
            return;
        }
        // Find current frame
        ...
        if (currentFrame.rotation) {
            ...
            const frameRotation = new THREE.Euler(((currentFrame.rotation.x || 0) * Math.PI) / 180, ((currentFrame.rotation.y || 0) * Math.PI) / 180, ((currentFrame.rotation.z || 0) * Math.PI) / 180);
            if (nextFrame && nextFrame.rotation && t > 0) {
                const nextRotation = new THREE.Euler(((nextFrame.rotation.x || 0) * Math.PI) / 180, ((nextFrame.rotation.y || 0) * Math.PI) / 180, ((nextFrame.rotation.z || 0) * Math.PI) / 180);
    ...
    updateProjectile(effect, progress) {
    ...
    updateMagicParticles(effect, elapsed) {
        if (!effect.particles) {
            return;
        }
    ...
    updateHitSplat(effect, progress) {
        if (!effect.mesh) {
            return;
        }
    ...
    updateDeathEffect(effect, elapsed) {
        if (!effect.particles) {
            return;
        }
        for (const particle of effect.particles) {
    ...
    cleanupEffect(effect) {
        ...
        // Remove mesh
        if (effect.mesh && visualSystem.removeMesh) {
            ;
            visualSystem.removeMesh(effect.mesh);
        }
        // Remove particles
        if (effect.particles) {
            for (const particle of effect.particles) {
                if (visualSystem.removeMesh) {
                    ;
                    visualSystem.removeMesh(particle);
                }
            }
        }
    }
...
exports.CombatAnimationSystem = CombatAnimationSystem;
//# sourceMappingURL=CombatAnimationSystem.js.map