...
const LootTableManager_1 = require("./loot/LootTableManager");
const DropCalculator_1 = require("./loot/DropCalculator");
const ItemRegistry_1 = require("./inventory/ItemRegistry");
class LootSystem extends sdk_1.System {
    constructor(world) {
        super(world);
        // Core management
        this.lootDrops = new Map();
        // Configuration
        this.LOOT_DESPAWN_TIME = 120000; // 2 minutes
        this.LOOT_VISIBLE_TIME = 60000; // 1 minute private, then public
        this.MAX_DROPS_PER_AREA = 100; // Performance limit
        this.lootTableManager = new LootTableManager_1.LootTableManager();
        this.dropCalculator = new DropCalculator_1.DropCalculator();
    ...
    async handleEntityDeath(entityId, killerId) {
        ...
        const vector3Position = Array.isArray(position)
    ...
    async createLootDrop(config) {
        ...
        const lootComponent = {
            type: 'loot',
            entity: null, // Loot doesn't have an entity yet
            data: {},
            items: stackedItems,
            owner: config.owner,
            spawnTime: Date.now(),
    ...
    calculateDistance(entity1, entity2) {
        ...
        const pos1 = Array.isArray(pos1Raw)
            ? { x: pos1Raw[0] || 0, y: pos1Raw[1] || 0, z: pos1Raw[2] || 0 }
            : pos1Raw;
        const pos2 = Array.isArray(pos2Raw)
