...
exports.DefaultRPGWorld = exports.ENTITY_VISUALS = void 0;
exports.initializeDefaultRPGWorld = initializeDefaultRPGWorld;
// Visual configurations for different entity types
exports.ENTITY_VISUALS = {
    // NPCs
...
class DefaultRPGWorld {
     ...
     */
    async initialize() {
        console.info('Initializing default RPG world...');
        ...
        console.info('Default RPG world initialized!');
    }
    async waitForSystems() {
        ...
        for (let i = 0; i < 50; i++) {
            // Max 5 seconds
            const allReady = requiredSystems.every(name => this.world[name]);
            if (allReady) {
                return;
            }
            await new Promise(resolve => setTimeout(resolve, 100));
        }
        console.warn('Some systems may not be ready');
    }
    async createSpawnArea() {
        console.info('Creating spawn area...');
        ...
        if (spawnPlatform) {
            ...
            this.addCubeVisual(spawnPlatform, exports.ENTITY_VISUALS.spawn_point);
            // Add name tag
            this.addNameTag(spawnPlatform, 'Spawn Point');
        }
    }
    async spawnTestNPCs() {
        console.info('Spawning test NPCs...');
        const npcSystem = this.world.npc;
        if (!npcSystem) {
        ...
        for (const spawn of spawns) {
            ...
            if (npc) {
                // Add cube visual
                this.addCubeVisual(npc, exports.ENTITY_VISUALS[spawn.name]);
                // Add name tag
                const npcComponent = npc.getComponent('npc');
                if (npcComponent) {
                    this.addNameTag(npc, npcComponent.name);
                }
                console.debug(`Spawned ${spawn.name} at ${JSON.stringify(spawn.position)}`);
            }
        }
    }
    async createServices() {
        ...
        if (bank) {
            ...
            this.addCubeVisual(bank, exports.ENTITY_VISUALS.bank_chest);
        ...
        if (shop) {
            ...
            this.addCubeVisual(shop, exports.ENTITY_VISUALS.shop_stall);
    ...
    async spawnGroundItems() {
        console.info('Spawning ground items...');
        const lootSystem = this.world.loot;
        if (!lootSystem) {
            console.error('Loot system not available');
            return;
        }
        ...
        for (const item of groundItems) {
            ...
            if (lootEntity) {
                ...
                // Add visual
                this.addCubeVisual(lootEntity, exports.ENTITY_VISUALS[item.visual] || exports.ENTITY_VISUALS.loot_pile);
                // Add name tag
    ...
    addCubeVisual(entity, config) {
        ...
        if (config === exports.ENTITY_VISUALS.coin || config === exports.ENTITY_VISUALS.loot_pile) {
    ...
    getItemDefinition(itemId) {
        const itemRegistry = this.world.inventory?.itemRegistry;
        if (itemRegistry) {
...
exports.DefaultRPGWorld = DefaultRPGWorld;
/**
 * Initialize default RPG world on server start
 */
async function initializeDefaultRPGWorld(world) {
    const defaultWorld = new DefaultRPGWorld(world);
    await defaultWorld.initialize();
}
//# sourceMappingURL=DefaultRPGWorld.js.map