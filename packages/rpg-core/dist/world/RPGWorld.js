...
exports.RPGWorldDefinition = void 0;
exports.initializeRPGWorld = initializeRPGWorld;
/**
 * Initialize an RPG world with default content
 */
async function initializeRPGWorld(world) {
    ...
    // Define spawn areas
    const tutorialIsland = { x: 3094, y: 0, z: 3107 };
    const lumbridge = { x: 3222, y: 0, z: 3218 };
    const varrock = { x: 3213, y: 0, z: 3428 };
    const falador = { x: 2965, y: 0, z: 3380 };
    const edgeville = { x: 3087, y: 0, z: 3496 };
    // Register NPCs
    ...
    npcSystem.spawnNPC(520, { x: lumbridge.x - 5, y: 0, z: lumbridge.z + 5 }) // General store
    ;
    npcSystem.spawnNPC(395, { x: lumbridge.x + 10, y: 0, z: lumbridge.z }) // Banker
    ;
    npcSystem.spawnNPC(278, { x: lumbridge.x, y: 0, z: lumbridge.z + 20 }) // Cook
    ;
    npcSystem.spawnNPC(3308, { x: tutorialIsland.x, y: 0, z: tutorialIsland.z }) // Tutorial Guide
    ;
    npcSystem.spawnNPC(520, { x: varrock.x, y: 0, z: varrock.z }) // General store
    ;
    npcSystem.spawnNPC(553, { x: varrock.x - 20, y: 0, z: varrock.z }) // Aubury (rune shop)
    ;
    npcSystem.spawnNPC(395, { x: varrock.x + 15, y: 0, z: varrock.z + 10 }); // Banker
    // Register bank booths
    const bankingSystem = world.systems.find(s => s.constructor.name === 'BankingSystem');
    if (bankingSystem) {
        ;
        bankingSystem.registerBankBooth('bank_lumbridge');
        bankingSystem.registerBankBooth('bank_varrock_west');
        bankingSystem.registerBankBooth('bank_varrock_east');
        bankingSystem.registerBankBooth('bank_falador_west');
        bankingSystem.registerBankBooth('bank_falador_east');
        bankingSystem.registerBankBooth('bank_edgeville');
    }
    ...
    if (settingsSystem && 'set' in settingsSystem) {
        ;
        settingsSystem.set('name', 'RuneScape RPG World');
        settingsSystem.set('description', 'A RuneScape-inspired RPG world in Hyperfy');
        settingsSystem.set('spawn', lumbridge);
    }
...
//# sourceMappingURL=RPGWorld.js.map