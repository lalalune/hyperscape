...
/**
 * Real Playwright manager that connects to actual Hyperia worlds
...
export class RealWorldPlaywrightManager {
  ...
  private async waitForWorldLoad(): Promise<void> {
    ...
    // Wait for canvas element (Hyperia uses canvas for 3D rendering)
  ...
  async getWorldInfo(): Promise<any> {
    return await this.page!.evaluate(() => {
      // Try to get Hyperia world information
      return {
        ...
        // Try to access Hyperia globals if available
        hyperia:
          typeof (window as { hyperia?: { version?: string } }).hyperia !==
          "undefined"
            ? {
                connected: true,
                version:
                  (window as { hyperia?: { version?: string } }).hyperia
                    ?.version || "unknown",
              }
            : { connected: false },
      };
