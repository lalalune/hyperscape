/**
 * Configuration constants for the Hyperia plugin
 * All configurable values should be defined here to avoid hardcoding
 */

export const LOCAL_DEV_HYPERIA_WS_URL = "ws://localhost:5556/ws";
export const PRODUCTION_HYPERIA_WS_URL =
  "wss://hyperia-production.up.railway.app/ws";
export const LOCAL_DEV_HYPERIA_API_BASE_URL = "http://localhost:5555";
export const PRODUCTION_HYPERIA_API_BASE_URL =
...
export const NETWORK_CONFIG = {
  DEFAULT_WS_URL:
    ...
    (isProductionRuntime()
      ...
      : LOCAL_DEV_HYPERIA_WS_URL),
  DEFAULT_API_BASE:
    process.env.PUBLIC_API_URL ||
    process.env.API_BASE ||
    (isProductionRuntime()
      ? PRODUCTION_HYPERIA_API_BASE_URL
...
export const SCRIPTED_AUTONOMY_CONFIG = {
  MODE: (process.env.HYPERIA_AUTONOMY_MODE || "llm").toLowerCase(),
  ROLE: process.env.HYPERIA_SCRIPTED_ROLE || "",
  SILENT_CHAT: process.env.HYPERIA_SILENT_CHAT === "true",
  FLEE_HEALTH_PERCENT: parseFloat(
    process.env.HYPERIA_FLEE_HEALTH_PERCENT || "30",
  ),
  MOB_LEVEL_MAX_ABOVE: parseInt(process.env.HYPERIA_MOB_LEVEL_MAX_ABOVE || "3"),
  MOB_LEVEL_MAX_BELOW: parseInt(process.env.HYPERIA_MOB_LEVEL_MAX_BELOW || "5"),
  RESOURCE_LEVEL_MAX_ABOVE: parseInt(
    process.env.HYPERIA_RESOURCE_LEVEL_MAX_ABOVE || "0",
  ),
  RESOURCE_LEVEL_MAX_BELOW: parseInt(
    process.env.HYPERIA_RESOURCE_LEVEL_MAX_BELOW || "10",
  ),
  RESOURCE_APPROACH_RANGE: parseFloat(
    process.env.HYPERIA_RESOURCE_APPROACH_RANGE || "40",
  ),
} as const;
