 ...
 */
export async function getHyperiaActions(
...
/**
 * Parse Hyperia world URL to extract world ID
 * @param url - Hyperia world URL
 * @returns World ID or null if invalid URL
 */
export function parseHyperiaWorldUrl(url: string): string | null {
  try {
    const urlObj = new URL(url);
    // Handle different Hyperia URL formats
    // e.g., https://hyperia.io/world-name or https://custom-domain.com
    const pathParts = urlObj.pathname.split("/").filter(Boolean);

    if (urlObj.hostname.includes("hyperia.io") && pathParts.length > 0) {
      return pathParts[0];
    }
 ...
 * @param entity - Entity object from Hyperia world
...
/**
 * Check if an entity is interactable based on Hyperia app system
...
export function isInteractableEntity(entity: InteractableEntityCheck): boolean {
  // Check for common interactive components in Hyperia
  return !!(
    entity.app ||
    entity.grabbable ||
    entity.clickable ||
...
/**
 * Convert Hyperia physics data to readable format
