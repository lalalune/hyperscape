/**
 * hyperia-client.ts - ElizaOS to Hyperia Bridge Client
 *
 * WebSocket client that connects ElizaOS AI agents to Hyperia game worlds.
 * Enables autonomous AI agents to join as players and interact with the game.
 *
 * **Purpose:**
 * Allows ElizaOS agents to:
 * - Connect to Hyperia servers as headless clients
 ...
 * **Usage:**
 * ```ts
 * const client = new HyperiaClientInterface({
 ...
 * ```
 *
 * **Referenced by:** plugin-hyperia action handlers, ElizaOS agent initialization
 */
...
/**
 * HyperiaClientInterface - ElizaOS Agent Game Client
 *
 * Connects ElizaOS AI agents to Hyperia game worlds as autonomous players.
 */
export class HyperiaClientInterface extends EventEmitter implements Client {
  ...
  async start(): Promise<void> {
    console.log(`[HyperiaClient] Starting connection to ${this.url}`);
    await this.connect();
  }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.on("error", (error) => {
          console.error("[HyperiaClient] WebSocket error:", error);
          reject(error);
        });
      } catch (error) {
        reject(error);
  ...
  private getAvailableActions(context: any): string[] {
    ...
    actions.push("HYPERIA_GOTO_ENTITY"); // Move to tasks
