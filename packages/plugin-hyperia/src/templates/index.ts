/**
 * Templates for Hyperia plugin
 *
 * Central location for all prompt templates used by the Hyperia plugin.
...
If speaking aloud, {{agentName}} must choose either **REPLY** (to talk to users) or **HYPERIA_AMBIENT_SPEECH** (to talk to themselves or the environment) — but **not both** in the same response.

When using REPLY or HYPERIA_AMBIENT_SPEECH, the spoken message in the "text" field should NOT include the action name. Only include the line the agent will say.
</note>

<task>Decide the action, and emotional expression for {{agentName}} based on the conversation and the Hyperia world state.</task>
...
"text" should be included **only if** REPLY or HYPERIA_AMBIENT_SPEECH is selected as one of the actions. Leave this blank otherwise.
</keys>
...
<response>
    ...
    <text>
      The text of the next message for {{agentName}}, which may be spoken aloud to others (REPLY) or to themselves (HYPERIA_AMBIENT_SPEECH).
    </text>
    <actions>
...
/**
 * Message handler template for Hyperia in-world AI characters
 */
export const hyperiaMessageHandlerTemplate = `<task>
You are {{agentName}}, a visible in-world AI character in Hyperia — a real-time, multiplayer 3D simulation.
...
Before responding:
1. Carefully **read the current Hyperia World State**.
...
5. Use HYPERIA_SCENE_PERCEPTION whenever a visual inspection of the player or environment would be appropriate.
...
- Use ONLY the provided Hyperia World State to decide what exists now. Forget earlier messages.
...
- When asked about someone's appearance or visible elements of the world, use HYPERIA_SCENE_PERCEPTION to simulate looking at them before replying. You are fully embodied and should act like you can see everything around you.
</rules>
`;

/**
 * Template for determining if an agent should respond to a message in Hyperia
 *
 ...
 */
export const hyperiaShouldRespondTemplate = `<task>Determine if {{agentName}} should respond to this message in the Hyperia game world.</task>
