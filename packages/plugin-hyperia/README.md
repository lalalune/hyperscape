# @hyperforge/plugin-hyperia

ElizaOS plugin for Hyperia - Connects AI agents to 3D multiplayer RPG worlds as real players.

## Overview

This plugin enables ElizaOS AI agents to play Hyperia as real players with full access to game mechanics:
...
### Service
- **HyperiaService**: Manages WebSocket connection to game server, maintains cached game state, executes commands
...
bun install @hyperforge/plugin-hyperia
```
...
```bash
# Hyperia server WebSocket URL
# Local dev default: ws://localhost:5556/ws
# Production default: wss://hyperia.gg/ws
HYPERIA_SERVER_URL=ws://localhost:5556/ws
...
1. **Connect** to Hyperia server on startup
...
- Tags: `['hyperia', 'combat', 'victory']`
- Tags: `['hyperia', 'resource', 'woodcutting', 'gathered']`
- Tags: `['hyperia', 'skill', 'levelup', 'fishing']`
...
5. **Type-Safe**: Full TypeScript types from both ElizaOS and Hyperia
6. **Modular**: Clean separation - Service → Providers → Actions

## Differences from Old Plugin

The previous `@elizaos/plugin-hyperia` was broken. This new implementation:
